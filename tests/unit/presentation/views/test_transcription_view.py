...
class TestTranscriptionView:
    ...
    def test_render_with_cache(
        self, mock_rerun, mock_success, mock_button, mock_selectbox, mock_markdown, mock_container, view, mock_presenter
    ):
        """キャッシュ利用時のレンダリングテスト"""
        # キャッシュを設定
    ...
    @patch("streamlit.rerun")
    def test_render_new_transcription(
        self, mock_rerun, mock_caption, mock_button, mock_radio, mock_markdown, mock_columns, view, mock_presenter
    ):
        """新規文字起こしのレンダリングテスト"""
    ...
    @patch("streamlit.success")
    @patch("streamlit.rerun")
    def test_show_processing_ui(
        self, mock_rerun, mock_success, mock_empty, mock_progress, mock_spinner, mock_button, view, mock_presenter
    ):
        """処理中UIのテスト"""
        # 処理中の状態を設定
        view.view_model.should_run = True
...
class TestShowTranscriptionControls:
    ...
    def test_show_transcription_controls_with_container(self, mock_view_class):
        ...
        use_cache, run_new, result = show_transcription_controls(
            video_path=video_path, api_key=api_key, container=mock_container
        )
    ...
    def test_show_transcription_controls_no_video(self, mock_error):
        ...
        use_cache, run_new, result = show_transcription_controls(container=mock_container)
