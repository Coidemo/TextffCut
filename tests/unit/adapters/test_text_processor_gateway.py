...
class TestTextProcessorGatewayAdapter:
    ...
    def test_find_differences_with_matching_text(self, gateway, mock_text_processor, sample_transcription_result):
        ...
        result = gateway.find_differences(
            transcription_result=sample_transcription_result, edited_text="文字起こしのテスト。"
        )
    ...
    def test_find_differences_with_added_text(self, gateway, mock_text_processor, sample_transcription_result):
        ...
        mock_text_processor.find_differences.return_value = {
            ...
            "differences": [("unchanged", "これはテストです。", 0), ("added", "追加テキスト。", -1)],
            "common_positions": [(0, 0)],
        }

        result = gateway.find_differences(
            transcription_result=sample_transcription_result, edited_text="これはテストです。追加テキスト。"
        )
    ...
    def test_get_time_ranges_from_differences(self, gateway, mock_text_processor, sample_transcription_result):
        ...
        # モックの設定
        mock_text_processor.get_time_ranges_for_segments.return_value = [(5.0, 10.0)]  # seg2の時間範囲

        result = gateway.get_time_ranges_from_differences(
            transcription_result=sample_transcription_result, differences=text_diff
        )
    ...
    def test_adjust_boundaries(self, gateway, mock_text_processor):
        ...
        # モックの設定
        mock_text_processor.adjust_boundaries.return_value = [(0.0, 5.5), (4.5, 10.0)]  # 0.5秒延長  # 0.5秒前倒し

        time_ranges = [TimeRange(start=0.0, end=5.0), TimeRange(start=5.0, end=10.0)]
        markers = {0: {"end_adjust": 0.5}, 1: {"start_adjust": -0.5}}

        result = gateway.adjust_boundaries(time_ranges=time_ranges, boundary_markers=markers)
    ...
    def test_split_into_sentences(self, gateway, mock_text_processor):
        ...
        mock_text_processor.split_into_sentences.return_value = ["これは最初の文です。", "これは二番目の文です。"]
    ...
    def test_split_text_by_separator(self, gateway, mock_text_processor):
        ...
        mock_text_processor.split_text_by_separator.return_value = ["パート1", "パート2", "パート3"]
        ...
        mock_text_processor.find_differences_with_separator.return_value = {
            "sections": [
                ...
                },
                {
                    ...
                    "differences": [("deleted", "セクション2", 0), ("added", "変更されたセクション2", -1)],
                    "common_positions": [],
                },
            ]
        }
    ...
    def test_get_time_ranges(self, gateway, mock_text_processor, sample_transcription_result):
        ...
        diff_info = {"common_positions": [(0, 0), (2, 1)]}

        result = gateway.get_time_ranges(
            transcription_result=sample_transcription_result, edited_text="一致するテキスト", diff_info=diff_info
        )
    ...
    def test_convert_to_legacy_diff_info(self, gateway):
        ...
        text_diff = TextDifference(
            ...
            differences=[(DifferenceType.UNCHANGED, "共通部分", 0), (DifferenceType.ADDED, "追加部分", -1)],
        )
    ...
    def test_convert_to_legacy_time_ranges(self, gateway):
        ...
        time_ranges = [TimeRange(start=0.0, end=5.0), TimeRange(start=10.0, end=15.0)]
    ...
    def test_error_handling_in_find_differences(self, gateway, mock_text_processor, sample_transcription_result):
        ...
        with pytest.raises(Exception, match="Processing error"):
            gateway.find_differences(transcription_result=sample_transcription_result, edited_text="テキスト")
    ...
    def test_empty_transcription_result(self, gateway, mock_text_processor):
        ...
        result = gateway.find_differences(transcription_result=empty_result, edited_text="新しいテキスト")
