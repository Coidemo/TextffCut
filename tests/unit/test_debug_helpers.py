...
from utils.debug_helpers import debug_words_status, debug_memory_usage, debug_file_info
...
class TestDebugWordsStatus:
    ...
    def test_with_segments_and_words(self, mock_get_logger):
        ...
        segment1 = SimpleNamespace(
            ...
            text='これはテスト用のセグメント1です。とても長い文章のテストです。'
        )
        segment2 = SimpleNamespace(
            words=['word4', 'word5'],
            text='セグメント2のテキスト'
        )
        ...
        mock_logger.info.assert_any_call("  セグメント0: 3words - これはテスト用のセグメント1です。とても長い文章のテストです...")
