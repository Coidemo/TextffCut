...
class TestSilenceInfo:
    ...
    def test_silence_info_creation(self):
        ...
        silence = SilenceInfo(start=10.0, end=20.0, duration=10.0)
...
class TestDetectSilenceRequest:
    ...
    def test_valid_request_creation(self):
        """有効なリクエストが作成できることを確認"""
        time_ranges = [TimeRange(start=0.0, end=10.0), TimeRange(start=20.0, end=30.0)]
    ...
    def test_request_with_defaults(self):
        ...
        request = DetectSilenceRequest(
            video_path=Path("/path/to/video.mp4"), time_ranges=[TimeRange(start=0.0, end=10.0)]
        )
...
class TestDetectSilenceUseCase:
    ...
    def use_case(self, mock_video_gateway, mock_file_gateway):
        ...
        return DetectSilenceUseCase(video_gateway=mock_video_gateway, file_gateway=mock_file_gateway)

    @pytest.fixture
    def sample_silence_data(self):
        """サンプルの無音検出結果を作成"""
        return [{"start": 2.0, "end": 3.5, "duration": 1.5}, {"start": 7.0, "end": 8.0, "duration": 1.0}]
    ...
    def test_validate_request_with_empty_time_ranges(self, use_case):
        """空の時間範囲でエラーになることを確認"""
        request = DetectSilenceRequest(video_path=Path("/path/to/video.mp4"), time_ranges=[])
    ...
    def test_validate_request_with_negative_padding(self, use_case):
        """負のパディングでエラーになることを確認"""
        request = DetectSilenceRequest(
            video_path=Path("/path/to/video.mp4"), time_ranges=[TimeRange(start=0.0, end=10.0)], padding=-0.1
        )
    ...
    def test_execute_with_padding(self, use_case, mock_video_gateway, mock_file_gateway):
        ...
        request = DetectSilenceRequest(
            video_path=Path("/path/to/video.mp4"), time_ranges=[TimeRange(start=0.0, end=10.0)], padding=0.5
        )
    ...
    def test_extract_audio_segments_method(self, use_case, mock_video_gateway, mock_file_gateway):
        ...
        audio_files = use_case._extract_audio_segments(
            video_path=Path("/path/to/video.mp4"), time_ranges=time_ranges, temp_dir=temp_dir
        )
