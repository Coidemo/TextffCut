...
class TestIntelligentAudioOptimizer:
    ...
    def mock_audio_info(self):
        """音声ストリーム情報のモック"""
        return {"sample_rate": 48000, "channels": 2, "duration": 60.0, "codec": "aac", "bitrate": "192k"}
    ...
    def test_analyze_audio_streams(self, optimizer, mock_video_path):
        ...
        with patch.object(optimizer, "_get_duration") as mock_get_duration:
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(stdout=mock_streams_output, stderr="", returncode=0)
    ...
    def test_build_conversion_command(self, optimizer):
        ...
        # 音声情報（実装に合わせた形式）
        audio_info = {"codec": "aac", "sample_rate": 48000, "channels": 2, "bit_rate": 192000}

        # 標準戦略でテスト
        cmd = optimizer._build_conversion_command(input_path, output_path, audio_info, strategy="standard")
        ...
        # アグレッシブ戦略でもテスト
        cmd_aggressive = optimizer._build_conversion_command(input_path, output_path, audio_info, strategy="aggressive")
    ...
    def test_get_optimization_summary(self, optimizer):
        ...
        optimizer.optimization_stats = [
            {"original_size_mb": 100, "optimized_size_mb": 10, "conversion_time_sec": 5.0},
            {"original_size_mb": 200, "optimized_size_mb": 30, "conversion_time_sec": 10.0},
        ]
    ...
    @pytest.mark.parametrize(
        "duration,file_size,expected_time",
        [
