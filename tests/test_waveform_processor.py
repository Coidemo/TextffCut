...
from core.waveform_processor import WaveformProcessor, ClipWaveformData, LIBROSA_AVAILABLE

# テスト用のダミー動画ファイルを作成するヘルパー関数
def create_dummy_audio_file(path: Path, sr=22050, duration=10, channels=1):
    path.parent.mkdir(parents=True, exist_ok=True)
    t = np.linspace(0., float(duration), int(sr * duration))
...
class TestWaveformProcessor(unittest.TestCase):
    ...
    def test_extract_waveforms_for_clips_success(self):
        ...
        result = self.processor.extract_waveforms_for_clips(
            str(self.dummy_video_path),
            time_ranges,
            samples_per_clip
        )
