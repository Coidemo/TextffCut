...
class TestFindTextDifferencesUseCase:
    ...
    def mock_transcription_result(self):
        ...
        segments = [
            ...
            TranscriptionSegment(
                ...
                words=[{"word": "削除される", "start": 2.0, "end": 3.0}, {"word": "部分", "start": 3.0, "end": 4.0}],
            ),
            TranscriptionSegment(
                id="3",
                text="これも残ります。",
                ...
                words=[{"word": "これも", "start": 4.0, "end": 5.0}, {"word": "残ります", "start": 5.0, "end": 6.0}],
            ),
        ]
    ...
    def test_successful_difference_detection(self, mock_gateway, mock_transcription_result, mock_text_difference):
        ...
        mock_gateway.get_time_ranges.return_value = [TimeRange(0.0, 2.0), TimeRange(4.0, 6.0)]
    ...
    def test_no_changes(self, mock_gateway, mock_transcription_result):
        ...
        use_case = FindTextDifferencesUseCase(mock_gateway)
        request = FindDifferencesRequest(
            original_text="同じテキスト", edited_text="同じテキスト", transcription_result=mock_transcription_result
        )
    ...
    def test_empty_texts_error(self, mock_gateway, mock_transcription_result):
        ...
        use_case = FindTextDifferencesUseCase(mock_gateway)
        request = FindDifferencesRequest(
            original_text="", edited_text="", transcription_result=mock_transcription_result
        )
    ...
    def test_no_segments_error(self, mock_gateway):
        ...
        use_case = FindTextDifferencesUseCase(mock_gateway)
        request = FindDifferencesRequest(
            original_text="テキスト", edited_text="編集後", transcription_result=mock_result
        )
...
class TestAdjustBoundariesUseCase:
    ...
    def time_ranges(self):
        """テスト用時間範囲"""
        return [TimeRange(0.0, 2.0), TimeRange(2.0, 4.0), TimeRange(4.0, 6.0)]
    ...
    def test_successful_boundary_adjustment(self, mock_gateway, time_ranges):
        ...
        request = AdjustBoundariesRequest(text_with_markers=text_with_markers, time_ranges=time_ranges)
    ...
    def test_empty_text_error(self, mock_gateway, time_ranges):
        """空のテキストでエラー"""
        use_case = AdjustBoundariesUseCase(mock_gateway)
        request = AdjustBoundariesRequest(text_with_markers="", time_ranges=time_ranges)
    ...
    def test_no_time_ranges_error(self, mock_gateway):
        """時間範囲なしでエラー"""
        use_case = AdjustBoundariesUseCase(mock_gateway)
        request = AdjustBoundariesRequest(text_with_markers="テキスト[1>]", time_ranges=[])
    ...
    def test_no_markers(self, mock_gateway, time_ranges):
        ...
        mock_gateway.apply_boundary_adjustments.return_value = (text_without_markers, time_ranges)  # 変更なし

        use_case = AdjustBoundariesUseCase(mock_gateway)
        request = AdjustBoundariesRequest(text_with_markers=text_without_markers, time_ranges=time_ranges)
