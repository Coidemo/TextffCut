...
class TestPhase4Integration:
    ...
    def test_transcription_and_text_processing_integration(self):
        """文字起こしとテキスト処理の統合テスト"""
        # モックの設定
        with patch('core.transcription.Transcriber') as mock_transcriber, \
             patch('adapters.gateways.text_processing.text_processor_gateway.LegacyTextProcessor') as mock_text_processor:
            ...
            legacy_diff = LegacyTextDifference(
                ...
                edited_text="これはテスト",
                common_positions=[
                    LegacyTextPosition(start=0, end=6, text="これはテスト")
                ],
                added_chars=set(),
            ...
            # 差分検出実行
            differences = text_gateway.find_differences(
                "これはテストです",
                "これはテスト"
            )
    ...
    def test_video_processing_integration(self):
        ...
        with patch('adapters.gateways.video_processing.video_processor_gateway.LegacyVideoProcessor') as mock_processor:
            ...
            # 残す範囲の計算
            mock_instance._calculate_keep_segments.return_value = [
                (0.0, 5.0),
                (8.0, 30.0)
            ]
            
            keep_ranges = video_gateway.calculate_keep_ranges(
                Duration(30.0),
                silence_ranges
            )
    ...
    def test_full_workflow_integration(self, temp_dir):
        ...
        with patch('core.transcription.Transcriber') as mock_transcriber, \
             patch('adapters.gateways.text_processing.text_processor_gateway.LegacyTextProcessor') as mock_text_processor, \
             patch('adapters.gateways.video_processing.video_processor_gateway.LegacyVideoProcessor') as mock_video_processor, \
            ...
            legacy_diff = LegacyTextDifference(
                ...
                common_positions=[
                    LegacyTextPosition(
                        start=0,
                        end=len(transcription_result.full_text),
