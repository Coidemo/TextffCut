...
from domain.entities import TranscriptionResult, TranscriptionSegment, Word, Char
...
class TestFCPXMLExportGatewayAdapter:
    ...
    def export_segments(self):
        ...
        return [
            ExportSegment(
                video_path=FilePath("/test/video1.mp4"),
                time_range=TimeRange(0.0, 10.0)
            ),
            ExportSegment(
                video_path=FilePath("/test/video2.mp4"),
                time_range=TimeRange(15.0, 25.0)
            )
        ]
...
class TestFCPXMLTimeMapper:
    ...
    def test_map_time_range_with_silence(self):
        """サイレンスありの時間マッピング"""
        silence_ranges = [
            TimeRange(5.0, 8.0),  # 3秒のサイレンス
            TimeRange(15.0, 18.0)  # 3秒のサイレンス
        ]
...
class TestSRTExportGatewayAdapter:
    ...
    def test_export_from_diff(self, gateway, transcription_result):
        """差分からのSRTエクスポートテスト"""
        time_ranges = [
            TimeRange(0.0, 3.0),  # 最初のセグメントを含む
            TimeRange(10.0, 15.0)  # どのセグメントも含まない
        ]
    ...
    def test_export_with_time_mapping(self, gateway, transcription_result):
        ...
        # 時間を半分に圧縮するマッピング
        time_mapping = [
            (TimeRange(0.0, 10.0), TimeRange(0.0, 5.0))
        ]
