...
class TestTextProcessorGatewayAdapter:
    ...
    def test_find_differences_success(self, mock_legacy_processor):
        ...
        legacy_diff = LegacyTextDifference(
            ...
            common_positions=[
                LegacyTextPosition(start=0, end=2, text="のテ")
            ],
            added_chars={"編", "集", "後"},
        ...
        # レガシーメソッドが呼ばれたことを確認
        mock_instance.find_differences.assert_called_once_with(
            original="元のテキスト",
            edited="編集後のテキスト"
        )
    ...
    def test_get_time_ranges_from_differences(self, transcription_result):
        ...
        with patch.object(
            ...
            '_convert_to_legacy_difference'
        ) as mock_convert_diff:
            with patch.object(
                TextProcessorGatewayAdapter,
                '_convert_to_legacy_transcription'
            ) as mock_convert_trans:
                # モックレガシー差分
                ...
                # 実行
                gateway = TextProcessorGatewayAdapter()
                time_ranges = gateway.get_time_ranges_from_differences(
                    differences, transcription_result
                )
