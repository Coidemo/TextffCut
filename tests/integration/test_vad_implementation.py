...
class TestVADImplementationIntegration(unittest.TestCase):
    ...
    def test_vad_based_transcription_flow(self):
        ...
        with patch('core.transcription_smart_boundary.SmartBoundaryTranscriber') as mock_transcriber_class, \
            ...
            mock_result.segments = [
                MagicMock(text="テストセグメント1", start=0.0, end=10.0, words=[{"word": "テスト", "start": 0.0, "end": 1.0}]),
                MagicMock(text="テストセグメント2", start=10.0, end=20.0, words=[{"word": "セグメント", "start": 10.0, "end": 11.0}])
            ]
    ...
    def test_api_mode_with_alignment(self):
        ...
        with patch('core.transcription.Transcriber') as mock_transcriber_class, \
    ...
    def test_vad_segment_30_second_constraint(self):
        ...
        for start, end in segments:
            ...
            self.assertLessEqual(
                duration, 30.0,
                f"セグメント ({start:.1f}-{end:.1f}) が30秒を超えています: {duration:.1f}秒"
            )
