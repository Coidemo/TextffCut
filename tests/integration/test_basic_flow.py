...
class TestBasicFlow:
    ...
    def test_video_processing_flow(self, mock_video_path):
        ...
        with patch("subprocess.run") as mock_run:
            # FFprobeのモック
            mock_run.return_value = Mock(stdout='{"format": {"duration": "60.0"}}', stderr="", returncode=0)
    ...
    def test_export_flow(self, tmp_path):
        ...
        segments = [ExportSegment(source_path=str(test_video), start_time=0.0, end_time=3.0, timeline_start=0.0)]
        ...
        with patch("core.export.VideoInfo") as mock_video_info:
            ...
            result = exporter.export(
                segments=segments, output_path=str(output_path), timeline_fps=30, project_name="Test Project"
            )
    ...
    def test_di_container_integration(self):
        ...
        # 各コンポーネントが取得できることを確認
        components = ["gateways", "use_cases", "services"]
