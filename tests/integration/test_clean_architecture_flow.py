...
class TestCleanArchitectureFlow:
    ...
    def test_text_editing_to_export_flow(self, app_container, sample_video_path, sample_transcription_result):
        ...
        # モックの設定
        with patch.object(
            app_container.gateways.fcpxml_export_gateway(),
            'export',
            return_value=Path("/tmp/output.fcpxml")
        ) as mock_export:
            # PresenterとUseCaseを取得
            presentation = app_container.presentation()
            ...
            # FCPXMLエクスポートを実行
            export_presenter.export_fcpxml(
                output_path=Path("/tmp/output.fcpxml"),
    ...
    def test_complete_workflow_with_silence_removal(self, app_container, sample_video_path, sample_transcription_result):
        ...
        with patch.object(
            ...
            app_container.gateways.video_export_gateway(),
            'export',
