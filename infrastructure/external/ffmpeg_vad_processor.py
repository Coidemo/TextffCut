...
class FFmpegVADProcessor(IVADProcessor):
    ...
    def detect_segments(
        ...
        try:
            ...
            # 無音区間を解析して音声区間を抽出
            vad_segments = self._parse_silence_output(
                result.stdout, 
                total_duration
            )
            
            # セグメントを適切なサイズに分割
            final_segments = self._split_segments(
                vad_segments, 
            ...
            logger.info(
                f"VAD検出: {len(vad_segments)}個の音声区間 → "
                f"{len(final_segments)}個のセグメントに分割"
            )
            ...
            # フォールバック：固定長分割
            return self._fallback_fixed_segments(
                audio_path, 
                max_segment_duration,
                min_segment_duration
            )
        ...
        for start, end in vad_segments:
            if end - start > max_duration:
                ...
                while current_start < end:
                    segment_end = min(
                        current_start + self.preferred_segment_duration, 
                        end
                    )
    ...
    def _fallback_fixed_segments(
        ...
        while current < duration:
            segment_end = min(
                current + self.preferred_segment_duration, 
                duration
            )
