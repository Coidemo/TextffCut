...
def test_video_processing_service():
    ...
    # モックVideoInfo
    mock_video_info = VideoInfo(path="/test/video.mp4", width=1920, height=1080, fps=30.0, duration=60.0, codec="h264")
    ...
    keep_ranges = [TimeRange(start=0.0, end=4.0), TimeRange(start=11.0, end=14.0)]
...
def test_text_editing_service():
    ...
    # テスト用セグメント
    original_segments = [
        TranscriptionSegmentV2(id="1", text="これはテストです", start=0.0, end=5.0, transcription_completed=True),
        TranscriptionSegmentV2(id="2", text="削除される部分", start=5.0, end=10.0, transcription_completed=True),
    ]
    ...
    for model_size in ["base", "medium", "large-v3"]:
        ...
        with patch("psutil.virtual_memory") as mock_vmem:
            ...
            with patch.object(diag.memory_monitor, "get_memory_usage", return_value=50.0):
                result = diag.run_diagnostics(segment_count=100, language="ja", test_alignment=False)
...
def test_service_integration():
    ...
    # シナリオ: 動画処理フロー
    print("\n1. 動画情報取得")
    mock_video_info = VideoInfo(path="/test/video.mp4", width=1920, height=1080, fps=30.0, duration=120.0, codec="h264")
    ...
    print("\n2. API料金計算")
    cost_result = config_service.calculate_api_cost(mock_video_info.duration / 60)  # 分に変換
    ...
    segments = [
        TranscriptionSegmentV2(
            id=f"seg_{i}", text=f"テキスト{i}", start=i * 10.0, end=(i + 1) * 10.0, transcription_completed=True
        )
        for i in range(10)
    ]
    ...
    with patch("psutil.virtual_memory") as mock_vmem:
        ...
        with patch.object(diag.memory_monitor, "get_memory_usage", return_value=40.0):
            diag_result = diag.run_diagnostics(segment_count=len(segments), language="ja", test_alignment=False)
