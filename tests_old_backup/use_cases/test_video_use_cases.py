...
class TestDetectSilenceUseCase:
    ...
    def mock_video_gateway(self):
        """モック動画ゲートウェイ"""
        gateway = Mock()
        gateway.extract_audio_segments.return_value = [FilePath("/temp/audio1.wav"), FilePath("/temp/audio2.wav")]
        gateway.detect_silence.return_value = [TimeRange(0.5, 1.5), TimeRange(3.0, 4.0)]  # 1秒の無音  # 1秒の無音
        gateway.calculate_keep_ranges.return_value = [TimeRange(0.0, 0.4), TimeRange(1.6, 2.9), TimeRange(4.1, 5.0)]
        return gateway
    ...
    def time_ranges(self):
        """テスト用時間範囲"""
        return [TimeRange(0.0, 5.0), TimeRange(10.0, 15.0)]
    ...
    def test_successful_silence_detection(self, mock_video_gateway, mock_file_gateway, time_ranges):
        ...
        request = DetectSilenceRequest(video_path=FilePath("/test/video.mp4"), time_ranges=time_ranges)
    ...
    def test_with_progress_callback(self, mock_video_gateway, mock_file_gateway, time_ranges):
        ...
        request = DetectSilenceRequest(
            video_path=FilePath("/test/video.mp4"), time_ranges=time_ranges, progress_callback=progress_callback
        )
    ...
    def test_file_not_found(self, mock_video_gateway, mock_file_gateway, time_ranges):
        """ファイルが存在しない場合"""
        use_case = DetectSilenceUseCase(mock_video_gateway, mock_file_gateway)
        request = DetectSilenceRequest(video_path=FilePath("/test/nonexistent.mp4"), time_ranges=time_ranges)
    ...
    def test_no_time_ranges(self, mock_video_gateway, mock_file_gateway):
        """時間範囲が空の場合"""
        use_case = DetectSilenceUseCase(mock_video_gateway, mock_file_gateway)
        request = DetectSilenceRequest(video_path=FilePath("/test/video.mp4"), time_ranges=[])
    ...
    def test_invalid_threshold(self, mock_video_gateway, mock_file_gateway, time_ranges):
        ...
        use_case = DetectSilenceUseCase(mock_video_gateway, mock_file_gateway)
        request = DetectSilenceRequest(
            video_path=FilePath("/test/video.mp4"), time_ranges=time_ranges, threshold=-70.0  # 範囲外
        )
...
class TestExtractVideoSegmentsUseCase:
    ...
    def mock_video_gateway(self):
        """モック動画ゲートウェイ"""
        gateway = Mock()
        gateway.get_video_info.return_value = {"duration": 60.0, "fps": 30.0, "width": 1920, "height": 1080}
        gateway.extract_segments.return_value = [FilePath("/temp/segment1.mp4"), FilePath("/temp/segment2.mp4")]
        return gateway
    ...
    def test_successful_extraction_with_combine(self, mock_video_gateway, mock_file_gateway):
        """正常な抽出（結合あり）"""
        # Arrange
        time_ranges = [TimeRange(0.0, 5.0), TimeRange(10.0, 15.0)]
    ...
    def test_single_segment_no_combine(self, mock_video_gateway, mock_file_gateway):
        ...
        mock_video_gateway.extract_segments.return_value = [FilePath("/temp/segment1.mp4")]

        use_case = ExtractVideoSegmentsUseCase(mock_video_gateway, mock_file_gateway)
        request = ExtractSegmentsRequest(
            video_path=FilePath("/test/video.mp4"), time_ranges=time_ranges, output_path=FilePath("/output/single.mp4")
        )
