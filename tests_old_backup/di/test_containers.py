...
from di.containers import create_container, get_container, reset_container
...
class TestApplicationContainer:
    ...
    def test_config_override(self):
        """設定のオーバーライドテスト"""
        # カスタム設定
        custom_config = DIConfig(environment="test", is_testing=True)
...
class TestDIConfig:
    ...
    def test_di_config_serialization(self):
        ...
        config = DIConfig(environment="test", is_testing=True)
...
class TestCustomProviders:
    ...
    def test_streamlit_session_provider_with_streamlit(self):
        ...
        with patch("streamlit.session_state", {"test_key": "test_value"}):
            provider = StreamlitSessionProvider(session_key="test_key", default_factory=lambda: "default")
    ...
    def test_conditional_provider(self):
        ...
        provider = ConditionalProvider(condition=condition, when_true=when_true, when_false=when_false)
