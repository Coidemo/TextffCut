...
def run_restart_test() -> None:
    ...
    if worker_0_info and worker_0_info.process.pid:
        ...
        if new_worker_0:
            print(f"Worker_0 restarted: new PID={new_worker_0['pid']}, " f"restarts={new_worker_0['restart_count']}")
        else:
    ...
    memory_intensive_task = {
        ...
        "segments": [
            {"start": i * 10.0, "end": (i + 1) * 10.0, "text": "メモリ圧迫テスト" * 1000}  # 大量のテキスト
        ...
        if memory_info.get("workers_under_pressure", 0) > 0:
            ...
            for worker in memory_info.get("workers", []):
                if worker["pressure_level"] != "normal":
                    print(f"  {worker['worker_id']}: {worker['pressure_level']} " f"({worker['memory_percent']:.1f}%)")
