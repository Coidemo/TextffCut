...
from typing import List, Optional, Dict, Any, reveal_type

# プロジェクトルートをパスに追加
sys.path.insert(0, str(Path(__file__).parent))

from core.types import (
    VideoPath, AudioPath, TimeSeconds, ModelSize,
    TranscriptionOptions, VideoMetadata, ProgressCallback,
    Result, Page, is_video_format, to_path
)
from core.models_typed import (
    TranscriptionSegmentV2, ProcessingMetadata,
    TranscriptionResultV2, WordInfo
)
...
def test_generic_types():
    ...
    success_result: Result[Dict[str, str]] = Result(
        success=True,
        data={'message': '成功しました'},
        metadata={'timestamp': '2025-06-08'}
    )
    print(f"Success result: {success_result.data}")
    
    error_result: Result[None] = Result(
        success=False,
        error="エラーが発生しました"
    )
...
def test_model_types():
    ...
    validation_result: tuple[bool, Optional[str]] = segment.validate_for_search()
