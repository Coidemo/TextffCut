...
def _parse_design_json(raw: dict) -> TitleImageDesign:
    ...
    for line_data in raw.get("lines", []):
        ...
        for seg_data in line_data.get("segments", []):
            ...
            segments.append(
                TitleTextSegment(
                    ...
                    font_size=_clamp(int(seg_data.get("font_size", 72)), 40, 120),
        ...
        lines.append(
            TitleLine(
                ...
                outer_outline_width=_clamp(int(line_data.get("outer_outline_width", 8)), 0, 10),
                inner_outline_color=str(line_data.get("inner_outline_color", "#FFFFFF")),
                inner_outline_width=_clamp(int(line_data.get("inner_outline_width", 0)), 0, 10),
            )
        )
    ...
    return TitleImageDesign(
        lines=lines,
        line_spacing=_clamp(int(raw.get("line_spacing", 10)), 0, 50),
        padding_top=_clamp(int(raw.get("padding_top", 60)), 0, 200),
    )
...
def _draw_gradient_fill(
    ...
    for row in range(h):
        ...
        for col in range(w):
            grad_img.putpixel((col, row), (r, g, b, 255))
