...
from use_cases.exceptions import TranscriptionError, ModelNotAvailableError
...
class TranscribeVideoUseCase(UseCase[TranscribeVideoRequest, TranscriptionResult]):
    ...
    def validate_request(self, request: TranscribeVideoRequest) -> None:
        ...
        if not request.video_path.validate_extension(valid_extensions):
            raise TranscriptionError(
                f"Invalid video/audio format: {request.video_path.extension}"
            )
        ...
        if request.model_size not in available_models:
            raise ModelNotAvailableError(
                f"Model '{request.model_size}' is not available. "
    ...
    def execute(self, request: TranscribeVideoRequest) -> TranscriptionResult:
        """文字起こしの実行"""
        self.logger.info(
            f"Starting transcription for: {request.video_path.name} "
            f"with model: {request.model_size}"
        )
        ...
        try:
            ...
            result = self.gateway.transcribe(
                ...
                skip_alignment=request.skip_alignment,
            ...
            raise TranscriptionError(
                f"Failed to transcribe video: {str(e)}",
                cause=e
            )
    
    def _try_load_cache(self, request: TranscribeVideoRequest) -> Optional[TranscriptionResult]:
        """キャッシュの読み込みを試行"""
        try:
            return self.gateway.load_from_cache(
                video_path=request.video_path,
                model_size=request.model_size
            )
        except Exception as e:
