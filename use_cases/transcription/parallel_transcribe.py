...
class ParallelTranscribeUseCase(UseCase[ParallelTranscribeRequest, TranscriptionResult]):
    ...
    def __init__(self, transcription_gateway: ITranscriptionGateway, video_gateway: IVideoProcessorGateway):
        super().__init__()
    ...
    def validate_request(self, request: ParallelTranscribeRequest) -> None:
        ...
        if request.chunk_duration < request.min_chunk_duration:
            raise TranscriptionError(
                f"Chunk duration ({request.chunk_duration}s) must be at least " f"{request.min_chunk_duration}s"
            )

    def execute(self, request: ParallelTranscribeRequest) -> TranscriptionResult:
        """並列文字起こしの実行"""
        self.logger.info(
            f"Starting parallel transcription for: {request.video_path.name} " f"with {request.num_workers} workers"
        )
        ...
        if request.progress_callback:
            request.progress_callback(f"Starting parallel transcription with {request.num_workers} workers...")
        ...
        try:
            ...
            raise TranscriptionError(f"Failed to transcribe video in parallel: {str(e)}", cause=e)
    ...
    def _get_video_info(self, video_path: FilePath) -> dict:
        ...
        try:
            ...
            raise TranscriptionError(f"Failed to get video information: {str(e)}", cause=e)
