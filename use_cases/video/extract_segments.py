...
from use_cases.exceptions import VideoProcessingError, SegmentCombineError
...
class ExtractVideoSegmentsUseCase(UseCase[ExtractSegmentsRequest, ExtractSegmentsResponse]):
    ...
    def execute(self, request: ExtractSegmentsRequest) -> ExtractSegmentsResponse:
        """セグメント抽出の実行"""
        self.logger.info(
            f"Starting segment extraction for {len(request.time_ranges)} ranges"
        )
        ...
        try:
            ...
            segment_files = self.video_gateway.extract_segments(
                ...
                output_dir=temp_dir,
                progress_callback=lambda p: request.progress_callback(0.1 + 0.6 * p) if request.progress_callback else None
            )
            
            # セグメント情報の作成
            segment_infos = []
            for i, (segment_file, time_range) in enumerate(zip(segment_files, request.time_ranges)):
            ...
            raise VideoProcessingError(
                f"Failed to extract segments: {str(e)}",
                cause=e
            )
    ...
    def _combine_segments(self, segment_files: List[FilePath], output_path: FilePath) -> None:
        """セグメントを結合"""
        try:
            self.video_gateway.combine_segments(
                segment_paths=segment_files,
                output_path=output_path
            )
        except Exception as e:
            raise SegmentCombineError(
                f"Failed to combine segments: {str(e)}",
                cause=e
            )
