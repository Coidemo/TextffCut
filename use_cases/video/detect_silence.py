...
from use_cases.exceptions import AudioExtractionError, SilenceDetectionError, VideoProcessingError
...
class DetectSilenceUseCase(UseCase[DetectSilenceRequest, DetectSilenceResponse]):
    ...
    def validate_request(self, request: DetectSilenceRequest) -> None:
        ...
        if not -60 <= request.threshold <= 0:
            raise VideoProcessingError(f"Threshold must be between -60 and 0 dB, got {request.threshold}")

        if request.min_silence_duration <= 0:
            raise VideoProcessingError("Minimum silence duration must be positive")

    def execute(self, request: DetectSilenceRequest) -> DetectSilenceResponse:
        """無音検出の実行"""
        self.logger.info(f"Starting silence detection for {len(request.time_ranges)} ranges")
        ...
        try:
            ...
            audio_files = self._extract_audio_segments(request.video_path, request.time_ranges, temp_dir)

            # 各セグメントから無音を検出
            all_silence_ranges = []
            silence_infos = []

            for i, (audio_file, time_range) in enumerate(zip(audio_files, request.time_ranges, strict=False)):
                if request.progress_callback:
                ...
                # セグメントの開始時間でオフセット
                offset_silence = [
                    TimeRange(start=time_range.start + sr.start, end=time_range.start + sr.end)
            ...
            # 全体の長さを計算
            total_duration = Duration(seconds=sum(tr.duration for tr in request.time_ranges))
            ...
            # 統計情報の計算
            silence_duration = Duration(seconds=sum(sr.duration for sr in all_silence_ranges))
            keep_duration = Duration(seconds=sum(kr.duration for kr in keep_ranges))
            ...
            raise VideoProcessingError(f"Failed to detect silence: {str(e)}", cause=e)

    def _extract_audio_segments(
        self, video_path: FilePath, time_ranges: list[TimeRange], output_dir: FilePath
    ) -> list[FilePath]:
        """音声セグメントを抽出"""
        try:
            return self.video_gateway.extract_audio_segments(
                video_path=video_path, time_ranges=time_ranges, output_dir=output_dir
            )
        except Exception as e:
            raise AudioExtractionError(f"Failed to extract audio segments: {str(e)}", cause=e)
