...
class ExportSRTRequest:
    ...
    silence_ranges: list[TimeRange] | None = None
...
class ExportSRTUseCase(UseCase[ExportSRTRequest, ExportSRTResponse]):
    ...
    def validate_request(self, request: ExportSRTRequest) -> None:
        ...
        if not request.output_path.extension.lower() == ".srt":
            raise ExportError(f"Invalid output format: {request.output_path.extension}. " "Expected .srt")
        ...
        if request.remove_silence and not request.silence_ranges:
            raise ExportError("silence_ranges required when remove_silence is True")

    def execute(self, request: ExportSRTRequest) -> ExportSRTResponse:
        """SRTエクスポートの実行"""
        self.logger.info(f"Starting SRT export with {len(request.transcription.segments)} segments")
        ...
        try:
            ...
            filtered_segments = self._filter_segments(request.transcription.segments, request.time_ranges)
            ...
            if request.remove_silence:
                adjusted_segments = self._adjust_for_silence_removal(filtered_segments, request.silence_ranges)
            else:
            ...
            entries = self._create_subtitle_entries(adjusted_segments, request.max_chars_per_line, request.max_lines)
            ...
            self.file_gateway.write_text(
                path=request.output_path, content=srt_content, encoding="utf-8-sig"  # BOM付きUTF-8
            )

            # 統計情報の計算
            total_duration = max((e.end_time for e in entries), default=0.0)
            ...
            raise ExportError(f"Failed to export SRT: {str(e)}", cause=e)

    def _filter_segments(
        self, segments: list[TranscriptionSegment], time_ranges: list[TimeRange] | None
    ...
    def _adjust_for_silence_removal(
        self, segments: list[TranscriptionSegment], silence_ranges: list[TimeRange]
    ) -> list[TranscriptionSegment]:
        """無音削除による時間調整"""
        # TimeMapperを使用して時間マッピング
        time_mapper = self.export_gateway.create_time_mapper(silence_ranges=silence_ranges)

        adjusted = []
        for segment in segments:
            # セグメントの時間範囲を調整
            adjusted_range = time_mapper.map_time_range(TimeRange(segment.start, segment.end))
    ...
    def _create_subtitle_entries(
        self, segments: list[TranscriptionSegment], max_chars_per_line: int, max_lines: int
    ) -> list[SubtitleEntry]:
        """字幕エントリを作成"""
        entries = []
        entry_index = 1
        ...
        for group in groups:
            ...
            entry = SubtitleEntry(
                index=entry_index, start_time=group[0].start, end_time=group[-1].end, text=combined_text, lines=lines
            )
    ...
    def _group_segments(self, segments: list[TranscriptionSegment], max_chars: int) -> list[list[TranscriptionSegment]]:
        """セグメントをグループ化"""
        groups = []
        current_group = []
        current_chars = 0
    ...
    def _split_into_lines(self, text: str, max_chars_per_line: int, max_lines: int) -> list[str]:
        """テキストを行に分割"""
        words = text.split()
        lines = []
        current_line = []
