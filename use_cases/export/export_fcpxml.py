...
class ExportFCPXMLUseCase(UseCase[ExportFCPXMLRequest, ExportFCPXMLResponse]):
    ...
    def validate_request(self, request: ExportFCPXMLRequest) -> None:
        ...
        if request.timeline_fps <= 0:
            raise ExportError(
                f"Invalid timeline FPS: {request.timeline_fps}"
            )
    
    def execute(self, request: ExportFCPXMLRequest) -> ExportFCPXMLResponse:
        """FCPXMLエクスポートの実行"""
        self.logger.info(
            f"Starting FCPXML export with {len(request.segments)} segments"
        )
        ...
        try:
            ...
            # 合計時間の計算
            total_duration = Duration(
                seconds=sum(segment.duration for segment in request.segments)
            )
            ...
            raise ExportError(
                f"Failed to export FCPXML: {str(e)}",
                cause=e
            )
