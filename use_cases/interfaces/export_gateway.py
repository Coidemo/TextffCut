...
class ExportSegment:
    ...
    def __init__(self, video_path: FilePath, time_range: TimeRange, label: str | None = None):
...
class TimeMapper:
    ...
    def map_time_range(self, time_range: TimeRange) -> TimeRange | None:
        """時間範囲をマッピング"""
        # 実装はアダプター層で行う
        pass
...
class IExportGateway(Protocol):
    ...
    def export(self, segments: list[ExportSegment], output_path: FilePath, **options: Any) -> None:
        """
        セグメントをエクスポート
