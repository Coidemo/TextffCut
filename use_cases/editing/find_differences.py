...
from domain.entities import TranscriptionResult, TextDifference
...
class FindTextDifferencesUseCase(UseCase[FindDifferencesRequest, FindDifferencesResponse]):
    ...
    def execute(self, request: FindDifferencesRequest) -> FindDifferencesResponse:
        ...
        try:
            ...
            self.logger.info(
                f"Difference detection completed. "
            ...
            if not time_ranges and request.edited_text:
                self.logger.warning(
                    "No time ranges found for edited text. "
                    "This may indicate a problem with word-level timestamps."
                )
