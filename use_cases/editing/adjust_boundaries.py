...
class AdjustBoundariesUseCase(UseCase[AdjustBoundariesRequest, AdjustBoundariesResponse]):
    ...
    def execute(self, request: AdjustBoundariesRequest) -> AdjustBoundariesResponse:
        ...
        try:
            ...
            cleaned_text, adjusted_ranges = self.gateway.apply_boundary_adjustments(
                text=request.text_with_markers, time_ranges=request.time_ranges
            )

            # 調整情報を抽出（独自実装）
            adjustments = self._extract_adjustments(request.text_with_markers, request.time_ranges)

            self.logger.info(f"Boundary adjustment completed. " f"Adjustments: {len(adjustments)}")

            return AdjustBoundariesResponse(
                cleaned_text=cleaned_text, adjusted_time_ranges=adjusted_ranges, adjustments=adjustments
            )
    ...
    def _extract_adjustments(
        self, text_with_markers: str, original_ranges: list[TimeRange]
        ...
        for adjustment_type, pattern in self.MARKER_PATTERNS.items():
            for match in pattern.finditer(text_with_markers):
                ...
                index = self._estimate_range_index(text_with_markers[:position], len(original_ranges))
                ...
                adjustments.append(
                    BoundaryAdjustment(
                        index=target_index, adjustment_type=adjustment_type, amount=amount, marker=marker
                    )
                )
