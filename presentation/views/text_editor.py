...
class TextEditorView:
        ...
        try:
            ...
            if not force_new and not append_to_existing:
                ...
                try:
                    ...
                    # 生成パラメータを表示
                    status_text.info(f"📈 生成設定: {buzz_clip_presenter.view_model.num_candidates}個の候補 / {buzz_clip_presenter.view_model.min_duration}-{buzz_clip_presenter.view_model.max_duration}秒 / OpenAI GPT-4o")
                    
                    # 生成実行
                    progress_callback(0.1, "APIへ接続中...")
                    logger.info(f"Calling generate_buzz_clips with {len(segments)} segments, force_new={force_new}, append_to_existing={append_to_existing}")
                    ...
                    logger.info(f"After generation: success={success}, candidates count={len(buzz_clip_presenter.view_model.candidates)}")
                    ...
                    if success and buzz_clip_presenter.view_model.candidates:
                        ...
                        if buzz_clip_presenter.view_model.token_usage:
                            ...
                            input_cost = (buzz_clip_presenter.view_model.token_usage.get("prompt_tokens", 0) / 1000000) * 5.0
                            output_cost = (buzz_clip_presenter.view_model.token_usage.get("completion_tokens", 0) / 1000000) * 20.0
                            total_cost = input_cost + output_cost
                            ...
                            logger.info(f"API cost: ${total_cost:.3f}, tokens: {buzz_clip_presenter.view_model.token_usage}")
                        ...
                        progress_bar.empty()
                        status_text.empty()
                        logger.error(f"Generation failed or no candidates. success={success}, candidates={buzz_clip_presenter.view_model.candidates}")
    ...
    def _render_buzz_clip_section(self) -> None:
        ...
        logger.info(f"[_render_buzz_clip_section] cache_checked in session: {'buzz_clip_cache_checked' in st.session_state}")
