...
class TranscriptionView:
    ...
    def render(self) -> None:
        ...
        if not use_cache:
            ...
            auto_blur_checked = st.checkbox(
                ...
                help=(
                    "コメント欄・UI 文字・チャンネルロゴ等を自動検出してぼかしを適用. "
    ...
    def _show_processing_ui(self) -> None:
        ...
        with st.spinner("文字起こし中..."):
            ...
            # auto_blur: 文字起こしと並列に動画内テキストぼかしを起動
            blur_executor = None
            blur_future = None
            ...
            if self.presenter.start_transcription(progress_callback):
                ...
                if blur_future is not None:
                    status_text.info("ぼかし処理の完了を待機中...")
                    try:
                        _blur_result = blur_future.result()
                        if _blur_result.cached:
                            st.info(f"🔒 ぼかし版動画 (cache): {_blur_result.blurred_path.name}")
                        else:
                            st.info(
                                f"🔒 ぼかし版動画を生成: {_blur_result.blurred_path.name} "
                                f"({_blur_result.duration_sec:.1f}s)"
                            )
                    except Exception as _e:  # noqa: BLE001
                        st.warning(f"⚠️ ぼかし処理エラー (文字起こしは完了): {_e}")
                    if blur_executor:
