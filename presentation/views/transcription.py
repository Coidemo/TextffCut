...
class TranscriptionView:
    ...
    def render(self, video_path: Path) -> Tuple[bool, bool, Optional[Any]]:
        """
        UIをレンダリング
        ...
        Returns:
            (use_cache, run_new, transcription_result)
        """
        # 動画情報で初期化
            ...
            with st.container(border=True):
                ...
                if st.button("💾 選択した結果を使用", type="primary", use_container_width=True):
                    if self.presenter.load_selected_cache():
                        use_cache = True
                        transcription_result = self.presenter.get_transcription_result()
                        st.success(f"✅ {selected_cache.mode}モード（{selected_cache.model_size}）の文字起こし結果を読み込みました！")
                        st.rerun()
        ...
        if not use_cache:
            ...
            if st.button(button_text, type=button_type, use_container_width=True):
                ...
                if self.view_model.use_api and not self.view_model.api_key:
                    ...
                    return use_cache, run_new, transcription_result
