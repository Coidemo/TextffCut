...
class VideoInputPresenter(BasePresenter[VideoInputViewModel]):
        ...
        def _refresh():
            try:
                ...
                for pattern in patterns:
                    matched_files = self.file_gateway.list_files(
                        FilePath(self.videos_dir), 
                        pattern
                    )
                    files.extend(matched_files)
                
                # ファイル名のみを抽出して重複を排除してソート
                file_names = sorted(list(set(Path(f).name for f in files)))
    ...
    def select_video(self, filename: Optional[str]) -> None:
        """
        動画を選択
