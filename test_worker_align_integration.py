...
def test_worker_align_with_diagnostics() -> None:
    ...
    config_data = {
        ...
        "language": "ja",
        "model_size": "medium",
        "config": {"transcription": {"language": "ja", "compute_type": "float32"}},
    }
    ...
    try:
        ...
        with patch("psutil.virtual_memory") as mock_vmem:
            ...
            with patch.object(diagnostics.memory_monitor, "get_memory_usage", return_value=50.0):
                result = diagnostics.run_diagnostics(segment_count=50, language="ja", test_alignment=False)
...
def test_memory_scenarios() -> None:
    ...
    config = Config()
    scenarios = [
        {"name": "十分なメモリ", "model": "medium", "available_gb": 16.0, "memory_percent": 30.0, "segments": 100},
        {"name": "メモリ制約あり", "model": "large-v3", "available_gb": 8.0, "memory_percent": 60.0, "segments": 200},
        {"name": "メモリ不足", "model": "large-v3", "available_gb": 4.0, "memory_percent": 80.0, "segments": 500},
    ]
    ...
    for scenario in scenarios:
        ...
        with patch("psutil.virtual_memory") as mock_vmem:
            mock_vmem.return_value = Mock(
                available=scenario["available_gb"] * 1024**3, total=32 * 1024**3, percent=scenario["memory_percent"]
            )

            with patch.object(diagnostics.memory_monitor, "get_memory_usage", return_value=scenario["memory_percent"]):
                result = diagnostics.run_diagnostics(
                    segment_count=scenario["segments"], language="ja", test_alignment=False
                )
