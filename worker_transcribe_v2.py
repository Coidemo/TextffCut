...
from core.constants import BatchSizeLimits, ChunkSizeLimits, ErrorMessages, MemoryThresholds, TranscriptionSegments
...
class BaseTaskHandler(ABC):
    ...
    def _create_transcriber(self):
        ...
        if self.config.transcription.use_api:
            ...
            logger.info(
                f"自動最適化パラメータ（初期）: チャンク={optimal_params['chunk_seconds']}秒, ワーカー={optimal_params['max_workers']}"
            )
...
class SeparatedModeHandler(BaseTaskHandler):
    ...
    def _process_alignment(self, result: Any) -> Any:
        ...
        # アライメント診断を実行
        optimal_batch_size = self._run_alignment_diagnostic(result, optimal_params, alignment_progress)
        ...
        aligned_segments = alignment_processor.align(
            segments, self.worker_config.video_path, result.language, progress_callback=main_alignment_progress
        )
    ...
    def _run_alignment_diagnostic(self, result: Any, optimal_params: dict, alignment_progress) -> int:
        ...
        if not diagnostic_result:
            ...
            if hasattr(result, "to_v2_format"):
                v2_result = result.to_v2_format()
                sample_segments = (
                    v2_result.segments[: TranscriptionSegments.SAMPLE_SEGMENTS_COUNT]
                    if len(v2_result.segments) >= TranscriptionSegments.SAMPLE_SEGMENTS_COUNT
                    else v2_result.segments
                )
            elif result.segments:
                sample_segments = (
                    result.segments[: TranscriptionSegments.SAMPLE_SEGMENTS_COUNT]
...
class TranscriptionWorker:
    ...
    def _create_task_handler(self) -> BaseTaskHandler:
        ...
        return handler_class(self.worker_config, self.memory_manager.optimizer, self.memory_manager.monitor)
    ...
    def _handle_general_error(self, e: Exception) -> None:
        ...
        # エラー結果を保存
        error_result = {"success": False, "error": str(e), "traceback": error_traceback}
