...
class VideoProcessorGatewayAdapter(IVideoProcessorGateway):
    ...
    def extract_audio_segments(
        self, video_path: FilePath, time_ranges: list[TimeRange], output_dir: FilePath | None = None
    ) -> list[FilePath]:
        """
        ...
        try:
            ...
            temp_wav_path = self._legacy_processor.extract_audio_for_ranges(
                video_path=str(video_path), time_ranges=time_ranges_tuples, output_path=out_dir
            )
            ...
            raise AudioExtractionError(f"Failed to extract audio segments from {video_path}: {str(e)}", cause=e)
    ...
    def detect_silence(
        ...
        try:
            ...
            for silence in silence_info_list:
                try:
                    silence_ranges.append(TimeRange(start=silence.start, end=silence.end))
            ...
            raise SilenceDetectionError(f"Failed to detect silence from {audio_path}: {str(e)}", cause=e)
    ...
    def calculate_keep_ranges(
        ...
        try:
            # SilenceInfoのリストに変換
            silence_info_list = [LegacySilenceInfo(start=sr.start, end=sr.end) for sr in silence_ranges]
    ...
    def extract_segments(
        ...
        try:
            ...
            raise VideoProcessingError(f"Failed to extract segments from {video_path}: {str(e)}", cause=e)
    ...
    def combine_segments(
        ...
        try:
            ...
            success = self._legacy_processor.combine_videos(
                segments=segment_paths_str, output=str(output_path), progress_callback=progress_wrapper
            )
            ...
            raise SegmentCombineError(f"Failed to combine {len(segment_paths)} segments: {str(e)}", cause=e)

    def get_video_info(self, video_path: FilePath) -> dict:
        """
        動画の情報を取得
    ...
    def remove_silence(
        ...
        """
        無音削除処理を実行し、残す部分の時間範囲を返す

        Args:
            video_path: 動画ファイルパス
            time_ranges: 処理対象の時間範囲
            silence_params: 無音検出パラメータ
    ...
    def extract_and_combine_audio(
        self, video_path: str, time_ranges: list[tuple[float, float]], output_path: str
    ) -> None:
        """
        指定された時間範囲の音声を抽出して結合
        ...
        try:
            ...
            logger.error(f"Failed to extract and combine audio: {e}")
            from use_cases.exceptions import AudioExtractionError

            raise AudioExtractionError(f"Failed to extract and combine audio: {str(e)}", cause=e)
