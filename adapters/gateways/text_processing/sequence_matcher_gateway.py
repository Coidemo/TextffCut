...
class SequenceMatcherTextProcessorGateway(ITextProcessorGateway):
    ...
    def normalize_with_position_tracking(
        self, text: str, markers: List[dict] = None
    ...
    def find_differences(
        ...
        try:
            ...
            for marker in sorted(context_markers_normalized, key=lambda m: m["start"], reverse=True):
                # {} を削除（中身は残す）
                comparison_text = (
                    comparison_text[: marker["start"]] + marker["content"] + comparison_text[marker["end"] :]
                )
            ...
            for marker in sorted(context_markers_normalized, key=lambda m: m["start"]):
                ...
                adjusted_markers.append({"content": marker["content"], "start": new_start, "end": new_end})
                offset += 2  # {}の分
            ...
            for tag, i1, i2, j1, j2 in opcodes:
                if tag == "equal":
                    ...
                    # この範囲を文脈マーカー位置で分割
                    # j1-j2は{}削除後のテキストでの位置
                    ranges_to_keep = self._split_range_excluding_markers(i1, i2, j1, j2, adjusted_markers)

                    # 分割された範囲を差分として追加
                    for orig_start, orig_end in ranges_to_keep:
                        if orig_start < orig_end:  # 有効な範囲のみ
                            actual_text = original_text[orig_start:orig_end]
                            differences.append((DifferenceType.UNCHANGED, actual_text, [(orig_start, orig_end)]))
                            logger.debug(f"UNCHANGED範囲追加: {orig_start}-{orig_end} '{actual_text}'")

                elif tag == "delete":
                    # 削除された部分
                    actual_text = original_text[i1:i2]
                    differences.append((DifferenceType.DELETED, actual_text, [(i1, i2)]))
            ...
            result = TextDifference(
                id=str(uuid4()), original_text=original_text, edited_text=edited_text, differences=differences
            )
    ...
    def _split_range_excluding_markers(
        self, orig_start: int, orig_end: int, edit_start: int, edit_end: int, markers: List[Dict]
    ) -> List[Tuple[int, int]]:
        """
    ...
    def extract_context_markers(self, text: str) -> List[dict]:
        ...
        for match in re.finditer(pattern, text):
            markers.append(
                {"content": match.group(1), "full_match": match.group(0), "start": match.start(), "end": match.end()}
            )
    ...
    def find_differences_with_separator(
        self, original_text: str, edited_text: str, separator: str = None
    ) -> TextDifference:
        """区切り文字を考慮した差分検出（未実装）"""
        ...
        for start_pos, end_pos in position_ranges:
            ...
            if 0 <= start_pos < len(char_array) and 0 <= end_pos <= len(char_array):
                ...
                if start_char.start is not None and end_time is not None:
                    ...
                    time_ranges.append(TimeRange(start=start_char.start, end=end_time))
