...
class TextProcessorGatewayAdapter(ITextProcessorGateway):
    ...
    def find_differences(
        ...
        try:
            ...
            logger.info(
                f"Text differences found: {unchanged_count} unchanged, "
                f"{added_count} added, {deleted_count} deleted"
            )
            ...
            raise TextProcessingError(
                f"Failed to find text differences: {str(e)}",
                cause=e
            )
    ...
    def get_time_ranges_from_differences(
        ...
        try:
            ...
            from use_cases.exceptions import TextProcessingError
            raise TextProcessingError(
                f"Failed to get time ranges from differences: {str(e)}",
                cause=e
            )
    ...
    def adjust_boundaries(
        ...
        try:
            ...
            adjusted_segments = self._simple_boundary_adjustment(
                text, time_ranges, transcription_segments, markers
            )
    ...
    def _convert_to_legacy_difference(
        self,
        differences: TextDifference
    ) -> LegacyTextDifference:
        """ドメインの差分情報をレガシー形式に変換"""
        ...
        for diff_type, text, _ in differences.differences:
            if diff_type == DifferenceType.UNCHANGED:
                ...
                if pos_in_original != -1:
                    pos = LegacyTextPosition(
                        start=pos_in_original,
                        end=pos_in_original + len(text),
                        text=text
                    )
