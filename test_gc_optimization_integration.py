...
def run_gc_optimization_test():
    ...
    manager = WorkerLifecycleManager(
        worker_func=transcription_worker_process, num_workers=2, max_restart_count=3, heartbeat_timeout=30.0
    )
    ...
    for level_name, segment_count, text_size in load_levels:
        ...
        task = {
            ...
            "segments": [
                {"start": i * 10.0, "end": (i + 1) * 10.0, "text": "テスト" * text_size} for i in range(segment_count)
            ],
