...
- **状態保持**: セッション状態で保持され、更新ボタンクリック時も状態が維持される

##### 更新ボタンのUI
- **テキスト**: 「更新」（絵文字なし）
- **幅**: コラム比率 1:3 で左側に配置
- **タイプ**: primary（赤色）
- **音声プレビュー**: 右側コラムに音声プレーヤーを配置（プレビュー時間表示はなし）
...
#### 21.1.1 チェックボックス状態保持の改善
- **問題**: 更新ボタンクリック時に境界調整モードのチェックボックスの状態が失われる
- **原因**: `st.rerun()`の直接呼び出しによりセッション状態が正しく処理されない
- **解決策**: フラグベース（`need_rerun`）のアプローチを採用
- **実装**:
  ```python
  # 更新ボタンの処理内
  st.session_state.need_rerun = True  # 直接rerunせずフラグを設定
