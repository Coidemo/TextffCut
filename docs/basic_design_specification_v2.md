...
flowchart TD
    ...
    subgraph "動的最適化"
        D --> E{メモリ容量}
        E -->|16GB以上| F[モデル: medium<br/>バッチ: 64<br/>float16]
        E -->|8-16GB| G[モデル: small<br/>バッチ: 32<br/>float16]
        E -->|4-8GB| H[モデル: base<br/>バッチ: 16<br/>int8]
        E -->|4GB未満| I[モデル: tiny<br/>バッチ: 8<br/>int8]
    end
...
**処理方針：**
- **VAD（Voice Activity Detection）による前処理**で無音部分を除去
- **Whisperの30秒制約**に合わせた最適なチャンク分割
- **メモリ容量に応じた動的最適化**（モデル選択、バッチサイズ、計算精度）
- **シングルパス推論**による効率的な並列処理
...
2. **アルゴリズムの選択**
   - **純粋LCS**：編集テキストが元テキストの10%以上の長さの場合
   - **ハイブリッド検索**：編集テキストが元テキストの10%未満の場合（例：90分動画から30秒の切り抜き）

3. **ハイブリッド検索アプローチ**（長大なテキストに対する短い検索の場合）
   - **特徴語抽出**：編集テキストから重要な単語を抽出（固有名詞、専門用語を優先）
   - **クラスタリング**：特徴語が密集している領域を特定（500文字以内）
   - **候補範囲でのLCS**：上位3つの候補範囲でLCSを実行し、最良の結果を選択
   - **効果**：計算量を500万から14万に削減（約35分の1）

4. **高度な機能**
   - **セパレータ対応**：`---`で区切られた複数セクションを順次処理
   - **文脈指定**：`{前後の単語}`形式で曖昧性を解消（例：「コーラが{一番}美味しい」）

5. **時間範囲の特定**
   - 各共通部分の開始文字と終了文字の位置を特定
   - 文字レベルのタイムスタンプから対応する時間を取得（WhisperXの日本語は1文字単位）
   - 開始時刻は最初の文字の開始時刻、終了時刻は最後の文字の終了時刻
...
**処理例2（長大なテキスト・ハイブリッド検索）：**
```
文字起こし全文（25,000文字・90分）:
"はい、それでは本日の講義を始めます...[中略]...バイアスがかかってしまわない..."

編集テキスト（200文字・0.8%）:
"バイアスがかかってしまわないために指針となる考え方..."

処理結果:
- アルゴリズム: ハイブリッド検索
- 特徴語: ["バイアス", "指針", "考え方"]
- 候補クラスタ: 3箇所検出、最高スコアの範囲でLCS実行
- 時間範囲: 2145.3秒 - 2187.6秒
```
...
3. **処理実行セクション**: `time_ranges_calculated`がTrueの場合
...
st.session_state.adjusted_time_ranges  # 調整された時間範囲（将来的な拡張用）
```
...
```mermaid
stateDiagram-v2
    [*] --> 動画選択/文字起こし: ワンページデザイン
    動画選択/文字起こし --> テキスト編集: 文字起こし完了
    テキスト編集 --> エクスポート設定: 差分検出実行
    エクスポート設定 --> 処理完了: エクスポート実行
    処理完了 --> [*]
    
    note right of 動画選択/文字起こし
        - 文字起こし結果を即座に表示
        - ナビゲーション問題を解決
    end note
```
...
#### 処理概要
動画ファイルから音声を抽出し、文字起こしを行う。VAD（Voice Activity Detection）による前処理と、メモリ容量に応じた動的最適化により、どんな環境でも安定動作を実現。ローカル版・API版ともに、文字起こし後にアライメント処理を実行し、単語レベルの精密なタイムスタンプを生成する。
...
**更新日**: 2025-07-05
**更新内容**: 
- ワンページデザインへの移行（ナビゲーション問題を解決）
- サイドバーを2タブ構成に変更（設定、ヘルプ）
- タイムライン編集機能を削除
- 画面遷移の簡素化