...
def test_mock_silence_removal():
    ...
    # テキスト分配
    segments = exporter._distribute_text_to_segments(
        text, original_range, mapped_ranges, words_with_timing
    )
    
    print("\n分配結果:")
    for i, (segment, time_range) in enumerate(zip(segments, mapped_ranges), 1):
        print(f"  セグメント{i}: '{segment}' [{time_range[0]:.1f}-{time_range[1]:.1f}秒]")
    
    # セグメントエントリを作成
    segment_entries = []
    for text_seg, (start_time, end_time) in zip(segments, mapped_ranges):
        if text_seg.strip():
