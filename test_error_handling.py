...
class TestErrorClasses(unittest.TestCase):
    ...
    def test_external_system_errors(self):
        """外部システムエラーのテスト"""
        # FFmpegエラー
    ...
    def test_words_field_missing_error(self):
        """wordsフィールド欠落エラーのテスト"""
...
class TestErrorHandler(unittest.TestCase):
    ...
    def test_handle_textffcut_error(self):
    ...
    def test_file_processing_scenario(self):
        ...
        def process_file(path: str):
            ...
            raise VideoProcessingError(
                "動画の処理に失敗しました",
                details={"path": path, "reason": "codec_not_supported"}
            )
        ...
        with self.assertRaises(VideoProcessingError) as cm:
            process_file("test.mp4")
        self.assertEqual(cm.exception.details['reason'], 'codec_not_supported')
    
    def test_memory_check_scenario(self):
        """メモリチェックシナリオ"""
