@echo off
chcp 65001 >nul
echo TextffCut v0.9.7 デバッグ版 v4
echo.
...
if %errorlevel% neq 0 (
    echo X 新版のDocker Composeが見つかりません
    echo.
    echo 旧版を試します: docker-compose version
    docker-compose version
    if %errorlevel% neq 0 (
        echo X docker-composeも見つかりません
        pause
        exit /b 1
    )
    echo OK docker-compose (旧版) を使用します
    set COMPOSE_CMD=docker-compose
) else (
    echo OK Docker Compose (新版) を使用します
    set COMPOSE_CMD=docker compose
)
...
if exist docker-compose-simple.yml (
    echo OK docker-compose-simple.yml: 存在
) else (
    echo X docker-compose-simple.yml: 見つかりません
    pause
    exit /b 1
)
...
if %errorlevel% neq 0 (
    echo X Dockerイメージがロードされていません
    echo.
    if exist textffcut_v0.9.7_docker.tar.gz (
        echo [イメージ読み込み] textffcut_v0.9.7_docker.tar.gz
        docker load -i textffcut_v0.9.7_docker.tar.gz
        if %errorlevel% neq 0 (
            echo X イメージの読み込みに失敗しました
            pause
            exit /b 1
        )
    ) else (
        echo X textffcut_v0.9.7_docker.tar.gz が見つかりません
        pause
        exit /b 1
    )
) else (
    echo OK Dockerイメージ: OK
)
