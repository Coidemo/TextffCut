...
def process_video_with_di(
    ...
    # 2. 無音検出
    silence_ranges = video_gateway.detect_silence(str(video_path), threshold=-35.0, min_duration=0.3)

    # 3. エクスポート
    export_result = export_gateway.export(
        video_path=str(video_path), time_ranges=[(0, 60)], output_path="output.fcpxml"  # 例：最初の60秒
    )

    return {"transcription": transcription_result, "silence_ranges": silence_ranges, "export_path": export_result}
...
def main():
    ...
    with st.sidebar:
        ...
        model_size = st.selectbox(
            "モデルサイズ", ["tiny", "base", "small", "medium", "large", "large-v3"], index=5, key="model_size"
        )
...
def use_cases_example(
    ...
    # 文字起こし
    transcribe_request = TranscribeVideoRequest(video_path=Path("video.mp4"), model_size="large-v3", language="ja")
    transcribe_response = transcribe_use_case.execute(transcribe_request)

    # 無音検出
    silence_request = DetectSilenceRequest(video_path=Path("video.mp4"), threshold=-35.0, min_duration=0.3)
