...
from di.providers import StreamlitSessionProvider, ConditionalProvider
...
class GatewayContainer(containers.DeclarativeContainer):
    ...
    # 動画処理ゲートウェイ
    video_processor_gateway = providers.Singleton(
...
class UseCaseContainer(containers.DeclarativeContainer):
    ...
    # 文字起こしユースケース
    transcribe_video = providers.Factory(
    ...
    export_fcpxml = providers.Factory(
        ExportFCPXMLUseCase,
        export_gateway=gateways.fcpxml_export_gateway,
        file_gateway=gateways.file_gateway
    )
    ...
    export_srt = providers.Factory(
        ExportSRTUseCase,
        srt_gateway=gateways.srt_export_gateway,
        file_gateway=gateways.file_gateway
    )
...
class ServiceContainer(containers.DeclarativeContainer):
    ...
    # テキスト編集サービス
    text_editing_service = providers.Singleton(
        TextEditingService,
        config=config.legacy_config
    )
...
class ApplicationContainer(containers.DeclarativeContainer):
    ...
    legacy_config = providers.Callable(
        lambda di_config: di_config.get_legacy_config(),
        di_config=config
    )
...
def create_container(
    ...
    override_providers: Optional[dict] = None
