...
class DIConfig:
    ...
    def update_from_streamlit_session(self, session_state: Dict[str, Any]) -> None:
        ...
        if "api_key" in session_state:
            setattr(self.legacy_config, "api_key", session_state["api_key"])
        ...
        if "use_api" in session_state:
            setattr(self.legacy_config, "use_api", session_state["use_api"])
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "DIConfig":
        ...
        if "legacy_config" in data:
            ...
            if "silence_threshold" in legacy_data:
                setattr(legacy_config.video, "silence_threshold", legacy_data["silence_threshold"])
            ...
            if "min_segment_duration" in legacy_data:
                setattr(legacy_config.video, "min_segment_duration", legacy_data["min_segment_duration"])
            # 動的属性
            if "api_key" in legacy_data:
                setattr(legacy_config, "api_key", legacy_data["api_key"])
            if "use_api" in legacy_data:
                setattr(legacy_config, "use_api", legacy_data["use_api"])
