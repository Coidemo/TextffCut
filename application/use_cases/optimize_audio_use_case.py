...
class OptimizeAudioUseCase(UseCase[OptimizeAudioRequest, OptimizeAudioResponse]):
    ...
    def __init__(
        self,
        audio_optimizer_gateway: IAudioOptimizerGateway,
    ...
    def execute(self, request: OptimizeAudioRequest) -> OptimizeAudioResponse:
        ...
        try:
            # 音声を最適化（常に実行）
            audio_data, optimization_info = self.audio_optimizer_gateway.prepare_audio(
                request.video_path
            )
            ...
            return OptimizeAudioResponse(
                success=False,
                error_message=f"音声最適化に失敗しました: {str(e)}"
            )