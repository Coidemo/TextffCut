    ...
    try:
        ...
        if remove_silence and time_mapper:
            ...
            success = srt_exporter.export_from_diff(
                ...
                transcription_result=transcription_result,
                output_path=str(output_path),
                max_chars_per_subtitle=get_srt_settings_from_session()["max_line_length"] * get_srt_settings_from_session()["max_lines"]
            )
...
def generate_export_paths(
    ...
    # XMLファイルのパス
    paths["xml"] = get_unique_path(
        project_path / f"{base_name}_TextffCut_{type_suffix}{xml_ext}"
    )
