...
def main():
    st.set_page_config(page_title="Phase 2 機能デモ", layout="wide", initial_sidebar_state="expanded")
    ...
    with st.sidebar:
        ...
        interaction.hover_info_enabled = st.checkbox("ホバー情報を表示", value=interaction.hover_info_enabled)
        interaction.boundary_adjustment_enabled = st.checkbox(
            "境界調整を有効化", value=interaction.boundary_adjustment_enabled
        )
        interaction.boundary_threshold = st.slider(
            "境界検出の閾値（秒）", min_value=0.05, max_value=0.5, value=interaction.boundary_threshold, step=0.05
        )
    ...
    with col1:
        ...
        fig = display.render_waveform(waveform_data, show_time_axis=True)

        if fig:
            # セグメント境界を追加
            boundaries = [st.session_state.demo_segment_start, st.session_state.demo_segment_end]
            ...
            selected_points = st.plotly_chart(
                fig, use_container_width=True, config=config, key="demo_waveform", on_select="rerun"
            )

            # クリックイベントの処理
            if selected_points and "selection" in selected_points:
                st.info("波形がクリックされました！")
                click_result = interaction.process_click_event(selected_points["selection"], waveform_data, boundaries)
    ...
    with col2:
        ...
        st.metric("開始時間", f"{st.session_state.demo_segment_start:.2f}秒")
        st.metric("終了時間", f"{st.session_state.demo_segment_end:.2f}秒")
        st.metric("長さ", f"{st.session_state.demo_segment_end - st.session_state.demo_segment_start:.2f}秒")
    ...
    with col1:
        ...
        def adjust_demo_end(value: float):
            ...
            st.session_state.demo_segment_end = max(
                st.session_state.demo_segment_start + 0.1, st.session_state.demo_segment_end
            )
            st.rerun()
    ...
    if st.button("境界調整パネルを表示"):
        with st.container():
            new_time, confirmed = interaction.create_adjustment_panel(
                st.session_state.demo_segment_end, st.session_state.demo_segment_start + 0.1, 10.0
            )
