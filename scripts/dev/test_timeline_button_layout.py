...
def test_timeline_editor_button_layout():
    ...
    with open(timeline_editor_path, encoding="utf-8") as f:
        content = f.read()
...
def test_documentation_consistency():
    ...
    with open(basic_design_path, encoding="utf-8") as f:
    ...
    with open(detailed_design_path, encoding="utf-8") as f:
        detailed_content = f.read()

    assert "[リセット] [編集を完了]" in detailed_content, "詳細設計書のボタン配置が更新されていません"
    assert (
        "[戻る]" not in detailed_content or "[キャンセル]" not in detailed_content
    ), "詳細設計書に古いボタンが残っています"
