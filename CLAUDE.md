...
### VADベース処理（実験的機能）
- **環境変数**: `TEXTFFCUT_USE_VAD=true` で有効化
- **動作**: 音声区間検出（VAD）を使用して30秒以内のセグメントに分割して処理
- **利点**: 大規模な動画でのメモリ効率向上
- **制限**: ローカルモードのみ（APIモードでは無効）
- **実装**: `OptimizedTranscriptionGatewayAdapter._transcribe_with_vad()`
...
1. **新機能開発は必ず新しいブランチで作業する**
   ...
   # 例: git checkout -b feature/buzz-clip-improvements
   ```
...
### 音声最適化機能の簡素化
1. **選択肢を削除し、常に最適化を実行**
   - 実証結果：品質100%維持、メモリ60-70%削減
   - 設定項目を削除してユーザビリティ向上
   - メンテナンス負荷の軽減

2. **技術的根拠**
   - 10分動画での品質比較：文字・単語レベルで100%一致
   - アライメント品質も完全に同一
   - 処理速度の差は無視できるレベル（圧縮時間1-2秒）
...
### テキスト処理のクリーンアーキテクチャ化
1. **スペース問題の根本解決**
   - TranscriptionResult.textプロパティを修正（セグメントをスペースなしで結合）
   - これにより、ユーザーの編集テキストとの比較が正確に

2. **新しいテキスト処理実装**
   - `TextDifferenceDetector`：レガシーコードを使わない差分検出ユースケース
   - `TimeRangeCalculator`：時間範囲の計算ユースケース
   - `SimpleTextProcessorGateway`：クリーンな実装のゲートウェイ
   - レガシーコードとドメインエンティティの混在を段階的に解消

3. **包括的なテスト**
   - 単体テスト、統合テスト、E2Eテストを実装
   - 追加文字削除機能の動作確認
