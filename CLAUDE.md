...
### YouTubeダウンロード機能のUI改善
1. **タブ形式の統合表示**
   - ローカルファイルとYouTubeダウンロードを同じセクション内のタブとして表示
   - 罫線（`---`）と「または」テキストを削除してUIを統一

2. **情報表示の改善**
   - 動画情報をアコーディオンから1行表示に変更（タイトル｜チャンネル名｜時間｜サイズ）
   - 「アップロード者」を「チャンネル名」に変更

3. **操作性の向上**
   - ダウンロードボタンの横にリセットボタンを配置（3:1の比率）
   - ダウンロード済みファイルの自動検出（yt-dlpの動作に基づく）
   - 不要なボタンを削除してシンプルに

4. **エラーハンドリングの強化**
   - DIコンテナ初期化時のエラーハンドリング追加
   - メモリリーク対策として1時間以上経過したYouTubeビューを自動クリーンアップ
