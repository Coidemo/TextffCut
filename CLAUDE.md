...
### OpenAI API使用時のタイムスタンプ補正

OpenAI Whisper APIは単語間のギャップを埋めるため、短い単語に異常に長いタイムスタンプを割り当てることがあります。

**自動補正条件：**
- 言語: 日本語のみ（`language='ja'`）
- 単語長: 2文字以下
- 継続時間: 0.5秒以上
- 位置: 検索範囲の最後の実質的な単語（句読点を除く）

**例：**
- 「です」が1.041秒として記録される → 前の単語の終了時刻を使用
- 実際の音声に合った正確な時間範囲での切り抜きが可能

**注意：**
- WhisperXローカル版では通常この問題は発生しません
- 英語など他言語では補正は適用されません
