...
def get_icon_svg() -> str:
    """
    テーマに応じたアイコンSVGを返す
    CSS media queriesを使用して自動的に色が切り替わる
    
    Returns:
        SVG文字列
    """
    # CSS media queriesを使用した動的な色設定
...
/* ダークモード用の色（白） - !importantで確実に適用 */
@media (prefers-color-scheme: dark) {
    .textffcut-logo .icon-dark { fill: #ffffff !important; }
    .textffcut-logo .icon-red { fill: #fd444d !important; }
}

/* 明示的なライトモード指定 */
@media (prefers-color-scheme: light) {
    .textffcut-logo .icon-dark { fill: #2a363b !important; }
    .textffcut-logo .icon-red { fill: #fd444d !important; }
}
</style>
