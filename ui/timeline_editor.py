...
def render_integrated_waveform_selector(segments: list, current_idx: int, video_path: str, service) -> int:
    ...
    for i, segment in enumerate(segments):
        ...
        # セグメント境界線
        if i > 0:
            fig.add_vline(x=start, line_color=colors["boundary_marker"], line_width=1, line_dash="dot", opacity=0.5)
...
def render_text_based_selector(segments: list, current_idx: int, video_path: str) -> int:
    ...
    for i, segment in enumerate(segments):
        ...
        with col2:
            # タイムスタンプとテキスト
            if is_selected:
                st.markdown('<div class="selected-segment"></div>', unsafe_allow_html=True)
        ...
        for col_idx in range(cols_per_row):
            ...
            if seg_idx < len(segments):
                ...
                with cols[col_idx]:
                    ...
                    if st.button(
                        f"{seg_idx + 1}", key=f"seg_btn_{seg_idx}", use_container_width=True, type=button_type
                    ):
...
def render_segment_editor(
    ...
    with col1:
        ...
        adjustments = [("-1s", -1.0), ("-0.1s", -0.1), ("-10ms", -0.01), ("+10ms", 0.01), ("+0.1s", 0.1), ("+1s", 1.0)]
...
def render_full_preview(service: TimelineEditingService):
    ...
    with st.spinner("プレビュー動画を生成中..."):
        ...
        try:
            ...
            # FFmpegで結合
            concat_cmd = ["ffmpeg", "-f", "concat", "-safe", "0", "-i", list_path, "-c", "copy", "-y", output_path]
