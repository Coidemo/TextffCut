...
def render_enhanced_timeline_editor(
    ...
    with col_right:
        if "timeline_data" in st.session_state and "selected_segment_id" in st.session_state:
            ...
            # 選択されたセグメントを取得
            selected_segment_data = next(
                (seg for seg in segments_data if seg["id"] == selected_segment_id), 
                None
            )
...
def render_waveform_tab(
    ...
    with st.spinner("波形データを読み込み中..."):
        ...
        # 波形の描画
        fig = waveform_display.render_waveform(
            waveform_data,
...
def register_keyboard_handlers(
    ...
    def previous_segment():
        if "selected_segment_id" in st.session_state and "timeline_data" in st.session_state:
            ...
            current_idx = next(
                (i for i, s in enumerate(segments) if s["id"] == st.session_state.selected_segment_id),
                -1
            )
            if current_idx > 0:
