...
class WaveformInteraction:
    ...
    def process_click_event(
        ...
        # 通常のクリック
        return {"action": "select_time", "time": click_time, "amplitude": click_amplitude}

    def _find_nearest_boundary(self, time: float, boundaries: list[float]) -> float | None:
        """最も近い境界を見つける"""
        if not boundaries:
            return None
    ...
    def create_interactive_waveform_config(self) -> dict[str, Any]:
        ...
        config = {
            ...
            "modeBarButtonsToAdd": [],
            "toImageButtonOptions": {"format": "png", "filename": "waveform", "height": 400, "width": 800, "scale": 2},
        }
    ...
    def create_adjustment_panel(self, boundary_time: float, min_time: float, max_time: float) -> tuple[float, bool]:
        """
        境界調整パネルを作成
        ...
        with col2:
            new_time = st.slider(
                "新しい位置", min_value=min_time, max_value=max_time, value=boundary_time, step=0.01, format="%.2f秒"
            )
...
class WaveformPlayback:
    ...
    def create_playback_controls(self) -> dict[str, Any]:
        ...
        with col4:
            st.progress(self.current_position, text=f"再生位置: {self.current_position:.1f}秒")

        return {"is_playing": self.is_playing, "position": self.current_position, "speed": self.playback_speed}
