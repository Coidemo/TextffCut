    ...
    with st.spinner("波形データを読み込み中..."):
        ...
        for i, (start, end) in enumerate(time_ranges):
            ...
            waveform_data = waveform_processor.extract_waveform(
                video_path, start, end, f"segment_{i+1}"
            )
            
            if waveform_data and "time" in waveform_data and "amplitude" in waveform_data:
                # 時間軸を調整（現在のオフセットを加算）
                adjusted_time = np.array(waveform_data["time"]) + current_x_offset
...
def render_segment_adjusters(time_ranges: List[Tuple[float, float]], service: TimelineEditingService):
    ...
    for i, segment_data in enumerate(segments):
        ...
        # 前後のセグメントの制約を取得
        min_start = 0 if i == 0 else updated_ranges[-1][1] + 0.1
        ...
        # セグメントのテキストを表示
        with st.expander(f"テキスト内容", expanded=False):
