...
from orchestrator.processing_state_manager import ProcessingStateManager, TranscriptionRecovery, check_and_recover_on_startup
...
def test_multiple_files_and_cleanup():
    ...
    for state in states:
        print(f"  - {Path(state['video_path']).name}: "
              f"{state['state']} ({state['progress']:.0%})")
...
def test_startup_recovery():
    ...
    try:
        ...
        for rec in recoverable:
            print(f"  - {Path(rec['video_path']).name}: "
                  f"{rec['state']} ({rec['progress']:.0%})")
