...
class WorkflowService(BaseService):
    ...
    def execute(self, **kwargs) -> ServiceResult:
        ...
        if workflow_type == "complete":
            ...
            return self.create_error_result(f"サポートされていないワークフロー: {workflow_type}", "ValidationError")
        ...
        try:
            ...
            if edited_text:
                ...
                diff_result = self.text_editing_service.find_differences(
                    original_segments=segments, edited_text=edited_text
                )
            ...
            # 出力パスの決定
            output_path = self._determine_output_path(video_file, settings.export_format, settings.output_dir)
            ...
            workflow_metadata = {
                ...
                "output_files": {"export": export_result.data["output_path"], "video": video_output_path},
            }

            self.logger.info(f"完全ワークフロー完了: {execution_time:.1f}秒, " f"{len(segments)} セグメント")

            return self.create_success_result(data=export_result.data, metadata=workflow_metadata)

        except Exception as e:
            self.logger.error(f"ワークフローエラー: {e}", exc_info=True)
            self._cleanup_temp_files()
            return self.wrap_error(ProcessingError(f"ワークフロー実行中にエラーが発生しました: {str(e)}"))
    ...
    def transcription_only_workflow(
        ...
        try:
            ...
            return self.wrap_error(ProcessingError(f"文字起こし実行中にエラーが発生しました: {str(e)}"))
    ...
    def export_only_workflow(
        ...
        try:
            ...
            return self.wrap_error(ProcessingError(f"エクスポート実行中にエラーが発生しました: {str(e)}"))
    ...
    def _process_video_segments(
        ...
        try:
            ...
            return self.create_success_result(
                data=output_path, metadata={"segments_processed": len(segments), "output_path": output_path}
            )
    ...
    def _determine_output_path(self, video_file: Path, export_format: str, output_dir: str | None = None) -> Path:
        """出力パスを決定
        ...
        """
        # 拡張子マッピング
        extension_map = {"fcpxml": ".fcpxml", "xmeml": ".xml", "edl": ".edl"}
