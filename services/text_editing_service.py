...
class TextEditingService(BaseService):
    ...
    def find_differences(
        self, original_segments: list[Segment], edited_text: str, merge_threshold: float = 0.5
    ) -> ServiceResult:
        """編集されたテキストとの差分を検出
        ...
        try:
            ...
            # 差分を検出
            diff = self.text_processor.find_differences(original_text, edited_text)
                    ...
                    if seg.start <= start < seg.end or seg.start < end <= seg.end:
                        ...
                        if overlap_end > overlap_start:
                            diff_segments.append(
                                Segment(start=overlap_start, end=overlap_end, text=seg.text, words=seg.words)
                            )
                            break
            ...
            return self.create_success_result(data=diff_segments, metadata=metadata)

        except ValidationError as e:
            return self.wrap_error(e)
        except Exception as e:
            self.logger.error(f"差分検出エラー: {e}", exc_info=True)
            return self.wrap_error(ProcessingError(f"差分検出中にエラーが発生しました: {str(e)}"))

    def merge_segments(
        self, segments: list[Segment], max_gap: float = 0.5, max_duration: float = 30.0
    ) -> ServiceResult:
        """近接したセグメントを結合
        ...
        try:
            ...
            self.logger.info(
                f"セグメント結合完了: {len(segments)} → {len(merged_segments)} " f"({merged_count} 個結合)"
            )

            return self.create_success_result(data=merged_segments, metadata=metadata)

        except Exception as e:
            self.logger.error(f"セグメント結合エラー: {e}", exc_info=True)
            return self.wrap_error(ProcessingError(f"セグメント結合中にエラーが発生しました: {str(e)}"))

    def split_segments(self, segments: list[Segment], max_duration: float = 10.0) -> ServiceResult:
        """長いセグメントを分割
        ...
        try:
            ...
            return self.create_success_result(data=split_segments, metadata=metadata)

        except Exception as e:
            self.logger.error(f"セグメント分割エラー: {e}", exc_info=True)
            return self.wrap_error(ProcessingError(f"セグメント分割中にエラーが発生しました: {str(e)}"))

    def format_segments_as_text(self, segments: list[Segment], include_timestamps: bool = True) -> ServiceResult:
        """セグメントをテキスト形式にフォーマット
        ...
        try:
            ...
            return self.create_success_result(data=formatted_text, metadata=metadata)

        except Exception as e:
            self.logger.error(f"テキストフォーマットエラー: {e}", exc_info=True)
            return self.wrap_error(ProcessingError(f"テキストフォーマット中にエラーが発生しました: {str(e)}"))

    def parse_text_with_timestamps(self, text: str) -> ServiceResult:
        """タイムスタンプ付きテキストをパース
        ...
        try:
            ...
            for line in text.strip().split("\n"):
                ...
                if match:
                    ...
                    segment = Segment(
                        start=parse_time(start_str), end=parse_time(end_str), text=segment_text.strip(), words=None
                    )
            ...
            metadata = {"parsed_segments": len(segments), "lines_processed": len(text.strip().split("\n"))}

            return self.create_success_result(data=segments, metadata=metadata)

        except Exception as e:
            self.logger.error(f"テキストパースエラー: {e}", exc_info=True)
            return self.wrap_error(ProcessingError(f"テキストパース中にエラーが発生しました: {str(e)}"))

    def _analyze_differences(
        self, original_segments: list[Segment], diff_segments: list[DiffSegment]
    ) -> dict[str, Any]:
        """差分の統計情報を分析
