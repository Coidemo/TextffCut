...
class ServiceResult:
    ...
    error_code: Optional[str] = None  # 新規追加
...
class BaseService(ABC):
    ...
    def validate_file_exists(self, file_path: str) -> Path:
        ...
        if not path.is_file():
            raise FileValidationError(
                f"ファイルではありません: {file_path}",
                details={"path": file_path, "is_directory": path.is_dir()}
            )
        return path
    ...
    def validate_directory_exists(self, dir_path: str) -> Path:
        ...
        if not path.exists():
            raise FileValidationError(
                f"ディレクトリが見つかりません: {dir_path}",
                details={"path": dir_path}
            )
        if not path.is_dir():
            raise FileValidationError(
                f"ディレクトリではありません: {dir_path}",
                details={"path": dir_path, "is_file": path.is_file()}
            )
        return path
