...
class TranscriptionService(BaseService):
    ...
    def execute(
        ...
        try:
            ...
            if use_api and not api_key:
                return self.create_error_result("APIキーが設定されていません", "ValidationError")

            # キャッシュチェック
            if use_cache:
                cache_key = self._generate_cache_key(video_file, model_size, use_api, language)
                cached_result = self._load_cache(cache_key)
                if cached_result:
                    self.logger.info(f"キャッシュから読み込み: {cache_key}")
                    return self.create_success_result(
                        data=cached_result, metadata={"from_cache": True, "cache_key": cache_key}
                    )

            # Transcriberの選択と設定
            transcriber = self._create_transcriber(use_api, api_key, separated_mode)
            ...
            return self.create_success_result(data=result, metadata=metadata)

        except ValidationError as e:
            return self.wrap_error(e)
        except Exception as e:
            self.logger.error(f"文字起こしエラー: {e}", exc_info=True)
            return self.wrap_error(ProcessingError(f"文字起こし処理中にエラーが発生しました: {str(e)}"))
    ...
    def load_from_cache(self, cache_path: str) -> TranscriptionResultV2 | None:
    ...
    def _validate_model_size(self, model_size: str, use_api: bool):
        ...
        if model_size not in valid_models:
            raise ValidationError(f"無効なモデルサイズ: {model_size}. " f"有効な値: {', '.join(valid_models)}")

    def _create_transcriber(self, use_api: bool, api_key: str | None, separated_mode: bool) -> Transcriber:
    ...
    def _generate_cache_key(self, video_file: Path, model_size: str, use_api: bool, language: str) -> str:
        """キャッシュキーを生成
    ...
    def _load_cache(self, cache_key: str) -> TranscriptionResultV2 | None:
        """キャッシュを読み込み
