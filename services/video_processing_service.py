...
class VideoProcessingService(BaseService):
    ...
    def remove_silence(
        ...
        try:
            ...
            # 時間範囲を作成
            time_ranges = [TimeRange(start=seg.start, end=seg.end) for seg in segments]
            ...
            keep_ranges = [TimeRange(start=start, end=end) for start, end in keep_ranges_tuples]

            # セグメントを調整
            adjusted_segments = self._adjust_segments_with_silence(segments, keep_ranges)

            # 統計情報を計算
            stats = self._calculate_silence_stats(segments, adjusted_segments, keep_ranges)
            ...
            return self.create_success_result(data=adjusted_segments, metadata=metadata)

        except ValidationError as e:
            return self.wrap_error(e)
        except Exception as e:
            self.logger.error(f"無音削除エラー: {e}", exc_info=True)
            return self.wrap_error(ProcessingError(f"無音削除中にエラーが発生しました: {str(e)}"))
        ...
        try:
            ...
            for i, segment in enumerate(segments):
                ...
                success = self.video_processor.extract_segment(
                    input_path=str(video_file), start=segment.start, end=segment.end, output_path=str(output_file)
                )
            ...
            self.logger.info(f"セグメント切り出し完了: {len(extracted_files)}/{len(segments)} ファイル")

            return self.create_success_result(data=extracted_files, metadata=metadata)

        except ValidationError as e:
            return self.wrap_error(e)
        except Exception as e:
            self.logger.error(f"セグメント切り出しエラー: {e}", exc_info=True)
            return self.wrap_error(ProcessingError(f"セグメント切り出し中にエラーが発生しました: {str(e)}"))

    def merge_videos(
        self, video_files: list[str], output_path: str, progress_callback: Callable[[float, str], None] | None = None
    ) -> ServiceResult:
        """複数の動画ファイルを結合
        ...
        try:
            ...
            try:
                # ffmpegで結合 - 正しいパラメータを使用
                success = self.video_processor.combine_videos(
                    input_files=video_files, output_file=output_path, progress_callback=wrapped_callback
                )
                ...
                self.logger.info(
                    f"動画結合完了: {video_info.duration:.1f}秒, " f"{metadata['output_size'] / 1024 / 1024:.1f}MB"
                )

                return self.create_success_result(data=output_path, metadata=metadata)
            ...
            return self.wrap_error(ProcessingError(f"動画結合中にエラーが発生しました: {str(e)}"))
    ...
    def get_video_info(self, video_path: str) -> ServiceResult:
        ...
        try:
            ...
            return self.create_success_result(data=info_dict, metadata={"file_path": str(video_file)})

        except ValidationError as e:
            return self.wrap_error(e)
        except Exception as e:
            self.logger.error(f"動画情報取得エラー: {e}", exc_info=True)
            return self.wrap_error(ProcessingError(f"動画情報取得中にエラーが発生しました: {str(e)}"))

    def _adjust_segments_with_silence(self, segments: list[Segment], keep_ranges: list[TimeRange]) -> list[Segment]:
        """無音削除結果に基づいてセグメントを調整
        ...
        for segment in segments:
            ...
            for keep_range in keep_ranges:
                ...
                if keep_range.start < segment.end and keep_range.end > segment.start:
                    # 重複部分を計算
                    overlap_start = max(keep_range.start, segment.start)
                    overlap_end = min(keep_range.end, segment.end)

                    if overlap_end > overlap_start:
                        segment_keep_ranges.append(TimeRange(start=overlap_start, end=overlap_end))
            ...
            if segment_keep_ranges:
                ...
                for keep_range in segment_keep_ranges:
                    adjusted_segment = Segment(
                        start=keep_range.start, end=keep_range.end, text=segment.text, words=segment.words
                    )
    ...
    def _calculate_silence_stats(
        self, original_segments: list[Segment], adjusted_segments: list[Segment], keep_ranges: list[TimeRange]
    ) -> dict[str, float]:
        """無音削除の統計情報を計算
        ...
        """
        # 元の総時間
        total_duration = sum(seg.end - seg.start for seg in original_segments)

        # 保持された時間
        kept_duration = sum(range.end - range.start for range in keep_ranges)
