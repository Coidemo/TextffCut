...
class BaseService(ABC):
    ...
    def wrap_error(self, error: Exception) -> ServiceResult:
        ...
        if isinstance(error, ServiceError):
            return ServiceResult(success=False, error=str(error), error_type=error.error_type)
        else:
            # 予期しないエラー
            self.logger.error(f"予期しないエラー: {error}", exc_info=True)
            return ServiceResult(success=False, error=str(error), error_type="UnexpectedError")
    ...
    def create_error_result(
        self, error: str, error_type: str = "Error", metadata: dict[str, Any] | None = None
    ) -> ServiceResult:
        """エラー結果の作成ヘルパー
        ...
        Returns:
            ServiceResult: エラー結果
        """
        return ServiceResult(success=False, error=error, error_type=error_type, metadata=metadata or {})
