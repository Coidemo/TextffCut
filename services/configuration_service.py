...
class ConfigurationService(BaseService):
    ...
    def calculate_api_cost(
        ...
        try:
            ...
            self.logger.info(
                f"API料金計算: {duration_minutes:.1f}分 → "
                f"{cost:.2f}{symbol} (${cost_usd:.3f})"
            )
    ...
    def validate_model_settings(
        ...
        try:
            ...
            if use_api:
                validation_result['recommendations'].append(
                    "APIモードは高速で安定していますが、料金が発生します"
                )
            else:
                if model_size == "base":
                    validation_result['recommendations'].append(
                        "baseモデルは高速ですが、精度が低い可能性があります"
                    )
                elif model_size == "large-v3":
                    validation_result['recommendations'].append(
                        "large-v3は最高精度ですが、処理時間が長くなります"
                    )
    ...
    def get_output_path(
        ...
        try:
            ...
            self.logger.info(
                f"出力パス生成: {output_name} "
                f"(タイプ: {process_type}, 形式: {output_format})"
            )
            
            return self.create_success_result(
                data=path_info,
                metadata={'path_generated': True}
            )
    ...
    def get_project_settings(
        ...
        try:
            ...
            return self.create_error_result(
                f"プロジェクト設定の生成中にエラーが発生しました: {str(e)}",
                "SettingsError"
            )
    ...
    def validate_api_key(self, api_key: str) -> ServiceResult:
        ...
        try:
            ...
            return self.create_success_result(
                data={'valid': True, 'masked_key': masked_key},
