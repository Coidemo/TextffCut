...
class VideoProcessingService(BaseService):
    ...
    def remove_silence(
        ...
        try:
            ...
            # 進捗通知のラップ
            wrapped_callback: ProgressCallback | None = self._wrap_progress_callback(progress_callback)

            # 時間範囲を作成
            time_ranges: list[TimeRange] = [TimeRange(start=seg.start, end=seg.end) for seg in segments]
            ...
            stats: dict[str, Any] = {
                ...
                "reduction_ratio": (
                    detection_result.removed_duration / detection_result.total_duration
                    if detection_result.total_duration > 0
                    else 0
                ),
            ...
            self.logger.info(
                f"無音削除完了: {stats['removed_duration']:.1f}秒削除 " f"({stats['reduction_ratio']:.1%}削減)"
            )

            return self.create_success_result(data={"segments": adjusted_segments}, metadata=stats)
    ...
    def get_video_info(self, video_path: VideoPath) -> ServiceResult:
        ...
        try:
            ...
            return self.create_success_result(data=metadata, metadata={"file_path": str(video_file)})
    ...
    def _adjust_segments_with_silence(
        self, segments: list[TranscriptionSegmentV2], keep_ranges: list[TimeRange]
        ...
        for segment in segments:
            ...
            for keep_range in keep_ranges:
                if self._ranges_overlap((segment.start, segment.end), (keep_range.start, keep_range.end)):
                    # 重複部分を新しいセグメントとして追加
    ...
    def _ranges_overlap(self, range1: tuple[TimeSeconds, TimeSeconds], range2: tuple[TimeSeconds, TimeSeconds]) -> bool:
        """時間範囲が重複するかチェック"""
        return range1[0] < range2[1] and range2[0] < range1[1]

    def _extract_single_segment(self, video_file: Path, segment: TranscriptionSegmentV2, output_file: Path) -> bool:
        """単一セグメントを切り出し（内部メソッド）"""
        try:
            # 実際の実装では FFmpeg を使用
            # ここではダミー実装
    ...
    def _wrap_progress_callback(self, callback: ProgressCallback | None) -> ProgressCallback | None:
        """進捗コールバックをラップ"""
        if callback is None:
            return None
