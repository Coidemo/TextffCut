...
def create_large_transcription(segment_count: int = 1000, words_per_segment: int = 20) -> TranscriptionResult:
    ...
    return TranscriptionResult(
        segments=segments,
        language="ja",
        original_audio_path="test_large.wav",
        model_size="base",
        processing_time=1.0
    )
...
def test_line_break_performance():
    ...
    test_texts = [
        ...
        "日本語と英語がmixedされたテキストも、適切にline breakできるかperformanceをチェックします。数字123や記号!?も含まれています。" * 10
    ]
...
def test_edge_cases():
    ...
    short_segments = []
    for i in range(10):
        short_segments.append(TranscriptionSegment(
            start=i * 0.1,
            end=(i + 1) * 0.1,
            text=f"短{i}",
    ...
    long_segment = TranscriptionSegment(
        ...
        text=long_text,
        words=[{"word": long_text, "start": 0.0, "end": 30.0}]
    )
