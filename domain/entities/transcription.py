...
class TranscriptionSegment:
    ...
    words: Optional[List[Union[Word, Dict[str, Any]]]] = None
    chars: Optional[List[Union[Char, Dict[str, Any]]]] = None
    ...
    def get_words_as_dicts(self) -> Optional[List[Dict[str, Any]]]:
        """wordsを辞書のリストとして取得（既存コードとの互換性）"""
        if not self.words:
            return None
        return [w.to_dict() if isinstance(w, Word) else w for w in self.words]
    
    def get_chars_as_dicts(self) -> Optional[List[Dict[str, Any]]]:
    ...
    def from_legacy_format(cls, data: Dict[str, Any]) -> "TranscriptionResult":
        ...
        for seg_data in data.get("segments", []):
            if isinstance(seg_data, dict):
                ...
                segments.append(TranscriptionSegment.from_legacy_format({
                    "text": seg_data.text,
