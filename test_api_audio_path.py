...
def test_audio_path_issue():
    ...
    if not os.path.exists(video_path):
        ...
        print("ffmpeg -f lavfi -i testsrc=duration=5:size=320x240:rate=30 -f lavfi -i sine=frequency=1000:duration=5 -c:v libx264 -c:a aac test_video.mp4")
        return
    ...
    print(f"   - APIモードでは original_audio_path (動画ファイル) が渡される")
    print(f"   - worker_align.py はこのパスから音声を読み込む必要がある")
