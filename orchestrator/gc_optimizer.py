...
class GarbageCollectionOptimizer:
    ...
    def optimize_based_on_memory_pressure(self, memory_percent: float) -> Optional[GCMetrics]:
        """メモリ圧迫度に基づいて最適化"""
    ...
    def adapt_thresholds(self):
        ...
        if avg_efficiency < self.adaptive_config["efficiency_target"]:
            # 閾値を下げる（頻度を上げる）
            for i in range(3):
                new_thresholds[i] = int(
                    current_thresholds[i] * (1 - self.adaptive_config["adjustment_factor"])
                )
    ...
    def get_optimization_suggestions(self) -> List[str]:
        ...
        if avg_uncollectable > 100:
            suggestions.append(f"収集不可能オブジェクトが多い（平均{avg_uncollectable:.0f}個）: "
                             "循環参照を確認してください")
        
        # 効率が低い場合
        avg_efficiency = sum(m.efficiency for m in recent_metrics) / len(recent_metrics)
        if avg_efficiency < 0.1:
            suggestions.append(f"GC効率が低い（{avg_efficiency:.2f}MB/ms）: "
                             "オブジェクトの生成パターンを見直してください")
        
        # GC時間が長い場合
        avg_duration = sum(m.duration for m in recent_metrics) / len(recent_metrics)
        if avg_duration > 100:  # 100ms以上
            suggestions.append(f"GC実行時間が長い（平均{avg_duration:.0f}ms）: "
                             "世代別GCの頻度調整を検討してください")
...
def optimize_for_transcription() -> GarbageCollectionOptimizer:
    ...
    optimizer.adaptive_config.update({
        "efficiency_target": 1.0,  # より高い効率目標
        "adjustment_factor": 0.15  # より積極的な調整
    })
...
def test_gc_optimizer():
    ...
    for memory_percent, description in test_scenarios:
        ...
        if metrics:
            print(f"GC executed: collected={metrics.collected}, "
