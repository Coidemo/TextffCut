...
from core.models import TranscriptionSegmentV2, TranscriptionResultV2
from core.transcription import Transcriber
from core.constants import ProcessingDefaults, PerformanceSettings
from utils.logging import BuzzClipLogger
from utils.cleanup import TempFileManager
from core.error_handling import (
    ErrorHandler, ProcessingError, ValidationError,
...
class TranscriptionWorker:
    ...
    def process_segment(
        ...
        try:
            ...
            result = self._transcribe_segment(
                segment, audio_file, chunk_duration
            )
            ...
            return self._handle_segment_error(
                segment_data.get("id", "unknown"), e
            )
    ...
    def process_all_segments(
        ...
        for i in range(0, len(segments), batch_size):
            ...
            self.logger.info(
                f"バッチ処理: {i+1}-{min(i+batch_size, len(segments))}/{len(segments)}"
            )
            
            # 並列処理
            batch_results = self._process_batch_parallel(
                batch, audio_path, chunk_duration
            )
    ...
    def _validate_segment(self, segment_data: Dict[str, Any]) -> TranscriptionSegmentV2:
        ...
        if missing_fields:
            raise ValidationError(
                f"必須フィールドが不足: {', '.join(missing_fields)}"
            )
        
        # セグメントオブジェクトに変換
        if isinstance(segment_data, dict):
            segment = TranscriptionSegmentV2.from_dict(segment_data)
        else:
        ...
        if segment.start < 0 or segment.end <= segment.start:
            raise ValidationError(
                f"無効な時間範囲: start={segment.start}, end={segment.end}"
            )
    ...
    def _transcribe_segment(
        ...
        try:
            # 一時ファイルの作成
            temp_audio = self.temp_manager.create_temp_file(
                suffix=f"_{segment.id}.wav",
                prefix="segment_"
            )
            
            # セグメント音声の抽出
            self._extract_segment_audio(
                audio_file, segment, temp_audio
            )
    ...
    def _extract_segment_audio(
        ...
        try:
            result = subprocess.run(
                ...
                timeout=ProcessingDefaults.SUBPROCESS_TIMEOUT
            )
            
            if result.returncode != 0:
                raise ProcessingError(
                    f"音声抽出に失敗: {result.stderr}"
                )
                
        except subprocess.TimeoutExpired:
            raise ProcessingError("音声抽出がタイムアウトしました")
    ...
    def _process_batch_parallel(
        ...
        for segment in batch:
            result = self.process_segment(
                segment, audio_path, chunk_duration
            )
