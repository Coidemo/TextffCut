...
class EnhancedMemoryMonitor(MemoryMonitor):
    ...
    def update_worker_status(
        self, worker_id: str, process_id: int, memory_mb: float, memory_percent: float
    ) -> WorkerMemoryStatus:
        """ワーカーのメモリ状態を更新"""
...
class MemoryOptimizer:
    ...
    def optimize_worker(self, worker_id: str, status: WorkerMemoryStatus) -> dict[str, Any]:
        ...
        return {"worker_id": worker_id, "actions_taken": actions_taken, "success": len(actions_taken) > 0}
    ...
    def should_restart_worker(self, worker_id: str) -> bool:
        ...
        if status.restart_count < 3 and status.pressure_level == MemoryPressureLevel.CRITICAL:
            ...
            critical_count = sum(
                1 for opt in recent_optimizations if opt["pressure_level"] == MemoryPressureLevel.CRITICAL
            )
            if critical_count >= 3:
                return True
...
class ProcessMemoryManager:
    ...
    def process_memory_report(self, msg: ProcessMessage) -> dict[str, Any] | None:
        """メモリレポートメッセージを処理"""
        ...
        # ワーカー状態を更新
        status = self.monitor.update_worker_status(worker_id, process.pid, memory_mb, memory_percent)
...
def test_memory_management():
    ...
    for worker_id, pid, memory_mb, memory_percent, description in test_scenarios:
        ...
        # メモリレポートメッセージを作成
        msg = ProcessMessage(
            msg_type=MessageType.MEMORY_STATUS, worker_id=worker_id, data={"pid": pid, "memory_percent": memory_percent}
        )
