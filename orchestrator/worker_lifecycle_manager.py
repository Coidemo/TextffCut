...
from orchestrator.memory_manager import ProcessMemoryManager, MemoryPressureLevel
...
class WorkerLifecycleManager:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        active_workers = sum(1 for w in self.workers.values() 
                           if w.state in [WorkerState.READY, WorkerState.PROCESSING])
    ...
    while time.time() - start_time < test_duration:
        ...
        if int(time.time() - start_time) % 5 == 0:
            ...
            for worker in status['workers']:
                print(f"  {worker['worker_id']}: {worker['state']} "
