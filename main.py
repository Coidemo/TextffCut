...
def render_text_edit_section(container):
    ...
    # バズクリップから遷移した場合の表示
    if st.session_state.get("show_text_edit_after_buzz", False):
        st.success(
            "✨ AIが選定した切り抜き候補が下記のテキストエディタに反映されました。必要に応じて編集してください。"
        )
        st.session_state["show_text_edit_after_buzz"] = False
    ...
    if transcription_result and video_path:
        ...
        # デバッグ情報を追加
        logger.info(f"[render_text_edit_section] transcription_result type: {type(transcription_result)}")
        logger.info(
            f"[render_text_edit_section] is TranscriptionResultAdapter: {isinstance(transcription_result, TranscriptionResultAdapter)}"
        )

        if isinstance(transcription_result, TranscriptionResultAdapter):
            actual_result = transcription_result.domain_result
            logger.info("[render_text_edit_section] Using domain_result from adapter")
        else:
            actual_result = transcription_result
            logger.info("[render_text_edit_section] Using transcription_result directly")

        logger.info(f"[render_text_edit_section] actual_result type: {type(actual_result)}")
        if hasattr(actual_result, "segments"):
            logger.info(f"[render_text_edit_section] segments count: {len(actual_result.segments)}")
            # テキストの最初の100文字をログ出力
            if actual_result.segments:
                full_text_preview = "".join(seg.text for seg in actual_result.segments[:3])[:100]
                logger.info(f"[render_text_edit_section] text preview: {full_text_preview}...")
