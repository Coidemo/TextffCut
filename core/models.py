...
class CharInfo:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "CharInfo":
        """辞書から生成"""
        return cls(char=data["char"], start=data.get("start"), end=data.get("end"), confidence=data.get("confidence"))
...
class TranscriptionSegmentV2:
    ...
    chars: list[CharInfo | dict[str, Any]] | None = None
    confidence: float | None = None
    ...
    def _convert_words_to_dict(self) -> list[dict[str, Any]]:
        ...
        for w in self.words:
            if isinstance(w, dict):
                ...
                # WordInfoオブジェクトの場合
                result.append({"word": w.word, "start": w.start, "end": w.end, "confidence": w.confidence})
        return result
    ...
    def _convert_chars_to_dict(self) -> list[dict[str, Any]]:
        ...
        for c in self.chars:
            if isinstance(c, dict):
                ...
                # CharInfoオブジェクトの場合
                result.append({"char": c.char, "start": c.start, "end": c.end, "confidence": c.confidence})
        return result
    ...
    def to_legacy_format(self) -> dict[str, Any]:
        ...
        return {"start": self.start, "end": self.end, "text": self.text, "words": legacy_words, "chars": legacy_chars}
...
class TranscriptionResultV2:
    ...
    def is_complete(self) -> bool:
        """処理が完了しているかチェック"""
        return (
            self.transcription_status == ProcessingStatus.COMPLETED
            and self.alignment_status == ProcessingStatus.COMPLETED
        )
    ...
    def validate_for_processing(self) -> tuple[bool, list[str]]:
        ...
        if segments_without_words:
            errors.append(f"{len(segments_without_words)}個のセグメントでwords情報が完全に欠落しています")

        if segments_with_invalid_words:
            errors.append(f"{len(segments_with_invalid_words)}個のセグメントでタイムスタンプが不完全です")
    ...
    def require_valid_words(self) -> None:
        ...
        主にUI表示前のチェックポイントで使用
        """
        from .exceptions import TranscriptionValidationError, WordsFieldMissingError

        is_valid, errors = self.validate_for_processing()

        if not is_valid:
            # wordsフィールド欠落の特別処理
            segments_without_words = [s for s in self.segments if not s.words or len(s.words) == 0]

            if segments_without_words:
                # サンプルテキストの取得
                sample_texts = [
                    s.text[:50] + "..." if s.text and len(s.text) > 50 else s.text for s in segments_without_words[:3]
                ]

                raise WordsFieldMissingError(segment_count=len(segments_without_words), sample_segments=sample_texts)
            else:
                # その他の検証エラー
...
class ProcessingRequest:
    ...
    # コールバック
    progress_callback: Any | None = None  # Callable[[float, str], None]
