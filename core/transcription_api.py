...
class APITranscriber:
    ...
    def _perform_alignment(
        ...
        try:
            ...
            align_model, metadata = whisperx.load_align_model(
                language_code=self.api_config.language, device="cpu"  # API版では基本的にCPU
            )

            # API結果をWhisperX形式に変換
            whisperx_segments = []
            for seg in segments:
                whisperx_segments.append({"start": seg.start, "end": seg.end, "text": seg.text})

            # アライメント実行（ローカル版と同じ処理）
            aligned_result = whisperx.align(
                whisperx_segments, align_model, metadata, audio, "cpu", return_char_alignments=True
            )
    ...
    def _transcribe_with_ffmpeg(
        ...
        try:
            # FFmpegでWAVに変換
            cmd = ["ffmpeg", "-i", audio_path, "-vn", "-ar", "16000", "-ac", "1", "-y", temp_wav_path]
    ...
    def _process_openai_response(self, response, audio_path: str | Path) -> TranscriptionResult:
        ...
        if hasattr(response, "segments") and response.segments:
            for seg in response.segments:
                segment = TranscriptionSegment(
                    start=seg.start, end=seg.end, text=seg.text, words=None  # アライメント処理なしの場合は None に設定
                )
                segments.append(segment)
        else:
    ...
    def _transcribe_with_separated_alignment(
        ...
        try:
            ...
            aligned_segments = self._align_in_subprocess(original_audio_path, api_segments, progress_callback)
            ...
            if perf_tracker:
                perf_tracker.end_tracking(
                    segments_processed=len(final_segments), api_chunks=len(chunks), alignment_chunks=len(final_segments)
                )
            ...
            if hasattr(response, "segments") and response.segments:
                for seg in response.segments:
                    if isinstance(seg, dict):
                        ...
                        segment = {"start": seg.start + start_offset, "end": seg.end + start_offset, "text": seg.text}
                    segments.append(segment)
            elif response.text.strip():
                # セグメント情報がない場合
                estimated_duration = len(response.text) / 20
                segment = {"start": start_offset, "end": start_offset + estimated_duration, "text": response.text}
                segments.append(segment)
    ...
    def _align_in_subprocess(
        ...
        try:
            ...
            # ワーカープロセスを実行
            cmd = [sys.executable, os.path.join(os.path.dirname(__file__), "..", "worker_align.py"), config_path]
            ...
            try:
                ...
                if return_code != 0:
                    ...
                    # エラー時は例外を発生させる
                    raise RuntimeError(
                        f"アライメント処理に失敗しました: ワーカープロセスが異常終了しました。\n{error_details}"
                    )
                            ...
                            for seg in aligned_segments:
                                if not seg.get("words"):
                                    seg["words"] = self._estimate_word_timestamps(
                                        seg.get("text", ""), seg.get("start", 0), seg.get("end", 0)
                                    )
