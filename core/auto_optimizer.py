...
class AutoOptimizer:
        ...
        try:
            ...
            if self.model_size == "large-v3" and available_gb < MemoryEstimates.LOW_MEMORY_GB:
                ...
                return {
                    ...
                    "align_chunk_seconds": min(
                        ChunkSizeLimits.EMERGENCY_MINIMUM, profile["initial_align_chunk_seconds"]
                    ),  # 最大5分
    ...
    def get_optimal_params(self, current_memory_percent: float) -> dict:
        ...
        # 調整方向の決定
        adjustment_type = self._determine_adjustment_type(current_memory_percent, memory_velocity)
    ...
    def _adjust_parameters(self, adjustment_type: str) -> dict:
        ...
        params["chunk_seconds"] = max(
            self.CHUNK_LIMITS["absolute_minimum"], min(self.CHUNK_LIMITS["maximum"], params["chunk_seconds"])
        )
        params["align_chunk_seconds"] = max(
            self.CHUNK_LIMITS["absolute_minimum"], min(self.CHUNK_LIMITS["maximum"] * 2, params["align_chunk_seconds"])
        )

        # ワーカー数調整
        if "worker_change" in adj:
            params["max_workers"] = max(
                WorkerLimits.MINIMUM, min(WorkerLimits.MAXIMUM, params["max_workers"] + adj["worker_change"])
            )
        ...
        if "batch_factor" in adj:
            ...
            params["batch_size"] = max(
                BatchSizeLimits.MINIMUM, min(BatchSizeLimits.MAXIMUM, params["batch_size"] + adj["batch_change"])
            )
    ...
    def _predict_optimal_params(self) -> dict:
        ...
        # 最小値の保証
        predicted_chunk_seconds = max(self.CHUNK_LIMITS["absolute_minimum"], predicted_chunk_seconds)
