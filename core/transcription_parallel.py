...
class ParallelTranscriber(SmartBoundaryTranscriber):
    ...
    def _calculate_optimal_workers(self) -> int:
        ...
        # 最終的なワーカー数（最小1、最大4）
        optimal_workers = min(max_workers_by_memory, max_workers_by_cpu, 4)  # 最大4並列（安定性のため）
    ...
    def _process_segments_parallel(
        self, segments: list[dict], progress_callback: Callable | None
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            # ジョブを投入
            future_to_segment = {executor.submit(process_segment_worker, seg): seg for seg in segments}
            ...
            for future in as_completed(future_to_segment):
                ...
                try:
                    ...
                    if progress_callback:
                        ...
                        progress_callback(progress, f"処理完了: {completed}/{total} セグメント")
...
def process_segment_worker(segment_data: dict) -> tuple[int, list[dict]]:
    ...
    try:
        ...
        model = whisperx.load_model(
            model_size, config["device"], compute_type=config["compute_type"], language=config["language"]
        )
        ...
        # アライメント処理を実行
        try:
            align_model, metadata = whisperx.load_align_model(language_code=config["language"], device=config["device"])

            aligned_result = whisperx.align(
                segments_data, align_model, metadata, audio, config["device"], return_char_alignments=True
            )
