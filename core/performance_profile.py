...
class FlexiblePerformanceProfile:
    ...
    def suggest_settings_based_on_history(self) -> Dict[str, Any]:
        ...
        # メモリエラーの頻度
        memory_errors = sum(
            1 for h in self.performance_history if "error" in h["metrics"] and "memory" in h["metrics"]["error"].lower()
        )
    ...
    def get_processing_config(self) -> Dict[str, Any]:
        ...
        # エラー履歴に基づく自動調整
        recent_errors = [h for h in self.performance_history[-5:] if not h["metrics"].get("success", True)]  # 最新5件
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        success_count = sum(1 for h in self.performance_history if h["metrics"].get("success", False))
