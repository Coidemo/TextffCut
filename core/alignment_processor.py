...
from .constants import AudioProcessing, BatchSizeLimits, MemoryEstimates, MemoryThresholds, TranscriptionSegments
...
class AlignmentProcessor(IAlignmentProcessor):
    ...
    def run_diagnostic(
        ...
        try:
            ...
            logger.info(
                f"モデルロード後メモリ: {model_loaded_memory:.1f}% (増加: {diagnostic_result['model_memory']:.1f}%)"
            )
            ...
            try:
                ...
                cmd = [
                    ...
                    audio_path,
                    "-t",
                    str(AudioProcessing.DIAGNOSTIC_DURATION),  # 60秒
                    "-ar",
                    str(AudioProcessing.SAMPLE_RATE),
                    "-ac",
                    str(AudioProcessing.CHANNELS),
                    "-f",
                    "wav",
                    temp_audio_path,
                ]
                ...
                total_duration = (
                    ...
                    else AudioProcessing.DEFAULT_DURATION_ESTIMATE
                )

                # 全音声データのメモリ使用量を推定
                diagnostic_result["audio_memory"] = (audio_memory_60s * total_duration) / 60
                logger.info(
                    f"音声メモリ推定: {diagnostic_result['audio_memory']:.1f}% (60秒分: {audio_memory_60s:.1f}%)"
                )
            ...
            if not sample_segments:
                sample_segments = self._generate_dummy_segments(
                    TranscriptionSegments.SAMPLE_SEGMENTS_COUNT
                )  # 10個のダミーセグメント
            ...
            if memory_increases:
                ...
                if avg_memory_per_segment > 0:
                    ...
                    max_safe_batch = max(
                        BatchSizeLimits.MINIMUM, min(max_safe_batch, BatchSizeLimits.LARGE)
                    )  # 1-16の範囲
                else:
                ...
                # 最適バッチサイズ（安全マージンを考慮）
                optimal_batch = max(
                    BatchSizeLimits.MINIMUM, int(max_safe_batch * MemoryEstimates.SAFETY_MARGIN)
                )  # 50%マージン
    ...
    def _generate_dummy_segments(self, count: int) -> list[TranscriptionSegmentV2]:
        ...
        for i in range(count):
            segment = TranscriptionSegmentV2(
                id=f"dummy_{i}",
                text=f"これはテスト用のダミーテキストです。セグメント番号{i}。"
                * TranscriptionSegments.DUMMY_SEGMENT_TEXT_REPEAT,  # 適度な長さ
        ...
        try:
            ...
            for batch_idx in range(total_batches):
                ...
                if progress_callback:
                    ...
                    progress_callback(progress, f"バッチ {batch_idx + 1}/{total_batches} を処理中")

                # バッチ処理
                batch_aligned = self._process_batch(batch_segments, audio_data, language)
            ...
            logger.info(f"アライメント完了: 成功={success_count}/{len(aligned_segments)}")
    ...
    def align_single_segment(
        ...
        try:
            ...
            if aligned_result and "segments" in aligned_result and aligned_result["segments"]:
                ...
                # wordsが空の場合、推定処理を使用
                if not words:
                    logger.warning("WhisperXがwordsを生成しませんでした。推定処理を使用します。")
    ...
    def estimate_timestamps(self, text: str, start_time: float, end_time: float) -> list[dict[str, Any]]:
        ...
        for word in words:
            word_info = {"word": word, "start": current_time, "end": current_time + word_duration}
    ...
    def _process_batch(
        self, segments: list[TranscriptionSegmentV2], audio_data: Any, language: str
    ) -> list[TranscriptionSegmentV2]:
        """バッチ単位でアライメント処理"""
        aligned_segments = []

        for segment in segments:
            aligned_segment = self.align_single_segment(segment, audio_data, language)
            aligned_segments.append(aligned_segment)

        return aligned_segments

    def _post_process_alignment(self, segments: list[TranscriptionSegmentV2]) -> list[TranscriptionSegmentV2]:
        """アライメント結果の後処理"""
    ...
    def _estimate_segment_timestamps(self, segment: TranscriptionSegmentV2) -> TranscriptionSegmentV2:
        """セグメントのタイムスタンプを推定"""
