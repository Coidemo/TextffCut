...
from .transcription import TranscriptionResult, TranscriptionSegment, Transcriber
from .video import VideoProcessor, VideoInfo, SilenceInfo
...
class SmartSplitTranscriber(Transcriber):
    ...
    def _transcribe_with_split(
        ...
        for i, (start, end) in enumerate(split_points):
            ...
            logger.info(f"セグメント {i+1}/{total_segments}: {start/60:.1f}分 - {end/60:.1f}分 ({segment_duration/60:.1f}分)")
            ...
            segment_result = self._process_segment(
                video_path, start, end, model_size, segment_callback
            )
    ...
    def _detect_silence_regions(self, video_path: str) -> List[SilenceInfo]:
        ...
        try:
            ...
            silences = self.video_processor.detect_silence_from_wav(
                str(temp_wav),
                noise_threshold=-40,  # 少し厳しめの閾値
                min_silence_duration=2.0  # 2秒以上の無音
            )
    ...
    def _calculate_split_points(
        ...
        silence_regions: List[SilenceInfo]
        ...
        for i in range(1, n_splits):
            ...
            # ±3分の範囲で最適な無音を探す
            best_silence = self._find_best_silence(
                silence_regions,
                target_time,
                search_window=3 * 60
            )
    ...
    def _process_segment(
        ...
        # Full VADで処理
        result = self._transcribe_segment_vad(
            segment_audio, model_size, progress_callback
        )
    ...
    def _transcribe_api_with_split_alignment(
        ...
        try:
            ...
            # 20分ごとに分割してアライメント処理
            segments = self._perform_split_alignment(
                audio, api_result.segments, duration, progress_callback
            )
