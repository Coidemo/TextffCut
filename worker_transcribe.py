...
def main() -> None:
    ...
    try:
        ...
        # デバッグ: APIモードの状態を出力
        logger.info(
            f"検証前の状態 - task_type: {task_type}, use_api: {config.transcription.use_api}, segments: {len(result.segments) if result.segments else 0}"
        )
        ...
        if result.segments and len(result.segments) > 0:
            ...
            logger.info(
                f"最初のセグメントの詳細 - text: '{first_seg.text[:50]}...', words: {len(first_seg.words) if first_seg.words else 'None'}"
            )
        ...
        if task_type != "transcribe_only" and result.segments:
            ...
            if not is_valid:
                ...
                for i, seg in enumerate(result.segments[:5]):  # 最初の5セグメントのみ
                    logger.error(
                        f"セグメント {i}: text='{seg.text[:30]}...', words={seg.words is not None}, words_count={len(seg.words) if seg.words else 0}"
                    )
